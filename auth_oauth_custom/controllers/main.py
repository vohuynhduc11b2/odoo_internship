...
class OAuthLogin(Home):
    def list_providers(self):
        try:
            providers = request.env['auth.oauth_custom.provider'].sudo().search_read([('enabled', '=', True)])
        except Exception:
            providers = []
        for provider in providers:
            return_url = request.httprequest.url_root + 'auth_oauth_custom/signin'
...
class OAuthController(http.Controller):
    ...
    @http.route('/auth_oauth_custom/signin', type='http', auth='none', readonly=False)
    ...
    def signin(self, **kw):
        ...
        try:
            # auth_oauth_custom may create a new user, the commit makes it
            # visible to authenticate()'s own transaction below
            _, login, key = request.env['res.users'].with_user(SUPERUSER_ID).auth_oauth_custom(provider, kw)
    ...
    @http.route('/auth_oauth_custom/oea', type='http', auth='none', readonly=False)
    def oea(self, **kw):
        ...
        with registry.cursor() as cr:
            try:
                ...
                provider = env.ref('auth_oauth_custom.provider_openerp')
            except ValueError:
                redirect = request.redirect(f'/web?db={dbname}', 303)
                redirect.autocorrect_location_header = False
                return redirect
            assert provider._name == 'auth.oauth_custom.provider'
