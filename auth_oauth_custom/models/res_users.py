...
class ResUsers(models.Model):
    ...
    oauth_provider_id = fields.Many2one('auth.oauth_custom.provider', string='OAuth Provider')
    ...
    def _auth_oauth_custom_rpc(self, endpoint, access_token):
        if self.env['ir.config_parameter'].sudo().get_param('auth_oauth_custom.authorization_header'):
    ...
    @api.model
    def _auth_oauth_custom_validate(self, provider, access_token):
        """ return the validation data corresponding to the access token """
        oauth_provider = self.env['auth.oauth_custom.provider'].browse(provider)
        validation = self._auth_oauth_custom_rpc(oauth_provider.validation_endpoint, access_token)
        if validation.get("error"):
            raise Exception(validation['error'])
        if oauth_provider.data_endpoint:
            data = self._auth_oauth_custom_rpc(oauth_provider.data_endpoint, access_token)
        ...
        # used (here, in _generate_signup_values, and in _auth_oauth_custom_signin)
    ...
    @api.model
    def _auth_oauth_custom_signin(self, provider, validation, params):
    ...
    @api.model
    def auth_oauth_custom(self, provider, params):
        ...
        validation = self._auth_oauth_custom_validate(provider, access_token)

        # retrieve and sign in user
        login = self._auth_oauth_custom_signin(provider, validation, params)
        if not login:
            raise AccessDenied()
