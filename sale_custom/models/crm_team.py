...
class CrmTeam(models.Model):
    ...
    def _compute_quotations_to_invoice(self):
        query = self.env['sale_custom.order']._where_calc([
            ('team_id', 'in', self.ids),
            ('state', 'in', ['draft', 'sent']),
        ])
        self.env['sale_custom.order']._apply_ir_rules(query, 'read')
        select_sql = SQL("""
    ...
    def _compute_sales_to_invoice(self):
        sale_order_data = self.env['sale_custom.order']._read_group([
            ('team_id', 'in', self.ids),
    ...
    def _graph_get_model(self):
        if self._in_sale_scope():
            return 'sale_custom.sale_report'
    ...
    def action_primary_channel_button(self):
        if self._in_sale_scope():
            return self.env["ir.actions.actions"]._for_xml_id("sale_custom.action_order_report_so_salesteam")
