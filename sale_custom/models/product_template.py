...
class ProductTemplate(models.Model):
    ...
    # 1) Field compute + inverse
    product_item_id = fields.Many2one(
        'oms.product.item', string='OMS Item',
        compute='_compute_product_item_id',
        inverse='_inverse_product_item_id',
        store=True, index=True
    )

    oms_valid_for = fields.Boolean(
        string='OMS còn kinh doanh',
        compute='_compute_oms_valid_for',
        store=True, index=True
    )

    # --- Compute: map theo default_code -> item_code ---
    @api.depends('default_code')
    def _compute_product_item_id(self):
        Item = self.env['oms.product.item'].sudo()
        # gom mã để query 1 lần
        codes = { (p.default_code or '').strip() for p in self if (p.default_code or '').strip() }
        by_code = {}
        if codes:
            items = Item.search([('item_code', 'in', list(codes))])
            by_code = { i.item_code: i.id for i in items }
        for p in self:
            code = (p.default_code or '').strip()
            p.product_item_id = by_code.get(code, False)

    # --- Inverse: chọn OMS item -> sync default_code ---
    def _inverse_product_item_id(self):
        for p in self:
            if p.product_item_id:
                code = p.product_item_id.item_code
                if code and p.default_code != code:
                    p.default_code = code

    # --- Cờ còn KD ---
    @api.depends('product_item_id', 'product_item_id.valid_for')
    def _compute_oms_valid_for(self):
        for p in self:
            v = (p.product_item_id.valid_for or '').strip().upper()
            p.oms_valid_for = (v == 'Y')

    # --- Ràng buộc trùng mã OMS ---
    @api.constrains('product_item_id', 'default_code')
    def _check_unique_mapping(self):
        ...
        for p in self:
            ...
            if not code:
                continue
            dups = Item.search([('item_code', '=', code)], limit=2)
            if len(dups) > 1:
                raise ValidationError(_("Có nhiều OMS Item trùng mã tham chiếu '%s'. Hãy chuẩn hóa dữ liệu OMS trước khi map.") % code)
    ...
    def _check_sale_product_company(self):
        ...
        for target_company, products in products_by_compagny.items():
            ...
            so_lines = self.env['sale_custom.order.line'].sudo().search_read(
    ...
    def action_view_sales(self):
        action = self.env['ir.actions.actions']._for_xml_id('sale_custom.report_all_channels_sales_action')
        action['domain'] = [('product_tmpl_id', 'in', self.ids)]
        action['context'] = {
            'pivot_measures': ['product_uom_qty'],
            'active_id': self._context.get('active_id'),
            'active_model': 'sale_custom.sale_report',
    ...
    def _get_backend_root_menu_ids(self):
        return super()._get_backend_root_menu_ids() + [self.env.ref('sale_custom.sale_menu_root').id]
