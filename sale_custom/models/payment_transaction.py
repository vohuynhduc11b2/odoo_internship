...
class PaymentTransaction(models.Model):
    ...
    sale_order_ids = fields.Many2many('sale_custom.order', 'sale_order_transaction_rel', 'transaction_id', 'sale_order_id',
    ...
    def _post_process(self):
        ...
        for done_tx in self.filtered(lambda tx: tx.state == 'done'):
            ...
            auto_invoice = str2bool(
                self.env['ir.config_parameter'].sudo().get_param('sale_custom.automatic_invoice')
            )
            if auto_invoice:
            ...
            if auto_invoice:
                if (
                    str2bool(self.env['ir.config_parameter'].sudo().get_param('sale_custom.async_emails'))
                    and (send_invoice_cron := self.env.ref('sale_custom.send_invoice_cron', raise_if_not_found=False))
                ):
    ...
    def _check_amount_and_confirm_order(self):
        ...
        :rtype: a `sale_custom.order` recordset
        """
        confirmed_orders = self.env['sale_custom.order']
        for tx in self:
    ...
    def _cron_send_invoice(self):
        ...
        """
        if not self.env['ir.config_parameter'].sudo().get_param('sale_custom.automatic_invoice'):
            return
    ...
    def _compute_reference_prefix(self, provider_code, separator, **values):
        ...
        if command_list:
            ...
            orders = self.env['sale_custom.order'].browse(order_ids).exists()
