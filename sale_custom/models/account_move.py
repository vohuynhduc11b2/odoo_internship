...
class AccountMove(models.Model):
    ...
    def _action_invoice_ready_to_be_sent(self):
        ...
        send_invoice_cron = self.env.ref('sale_custom.send_invoice_cron', raise_if_not_found=False)
    ...
    def action_view_source_sale_orders(self):
        ...
        result = self.env['ir.actions.act_window']._for_xml_id('sale_custom.action_orders')
        if len(source_orders) > 1:
            result['domain'] = [('id', 'in', source_orders.ids)]
        elif len(source_orders) == 1:
            result['views'] = [(self.env.ref('sale_custom.view_order_form', False).id, 'form')]
    ...
    def _get_partner_credit_warning_exclude_amount(self):
        ...
        # (see _compute_credit_to_invoice and _compute_amount_to_invoice from 'sale_custom.order' )
    ...
    def _compute_partner_credit(self):
        ...
        for move in self.filtered(lambda m: m.is_invoice(include_receipts=True)):
            ...
            amount_to_invoice_currency = sum(
                sale_order_custom.currency_id._convert(
                    sale_order_custom.amount_to_invoice,
