...
class AccountMoveLine(models.Model):
    ...
    def _sale_create_reinvoice_sale_line(self):
        ...
        #   - value is either a sale_custom.order.line record (existing case), or an integer representing the index of the sale line to create in
        ...
        for move_line in self:
            ...
            if (
                ...
                map_entry_key = (sale_order_custom.id, move_line.product_id.id, price)  # cache entry to limit the call to search
                sale_line = existing_sale_line_cache.get(map_entry_key)
                if sale_line:  # already search, so reuse it. sale_line can be sale_custom.order.line record or index of a "to create values" in `sale_line_values_to_create`
                    ...
                    sale_line = self.env['sale_custom.order.line'].search([
        ...
        new_sale_lines = self.env['sale_custom.order.line'].create(sale_line_values_to_create)

        # build result map by replacing index with newly created record of sale_custom.order.line
        result = {}
        for move_line_id, unknown_sale_line in map_move_sale_line.items():
            ...
            elif isinstance(unknown_sale_line, models.BaseModel):  # already record of sale_custom.order.line
                result[move_line_id] = unknown_sale_line
        return result

    def _sale_determine_order(self):
        """ Get the mapping of move.line with the sale_custom.order record on which its analytic entries should be reinvoiced
            :return a dict where key is the move line id, and value is sale_custom.order record (or None).
        """
        return {}
    ...
    def _sale_prepare_sale_line_values(self, order, price):
        ...
        self.ensure_one()
        last_so_line = self.env['sale_custom.order.line'].search([('order_id', '=', order.id)], order='sequence desc', limit=1)
