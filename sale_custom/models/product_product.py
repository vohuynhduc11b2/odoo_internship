...
class ProductProduct(models.Model):
    ...
    oms_valid_for = fields.Boolean(related='product_tmpl_id.oms_valid_for', store=True, index=True)
    ...
    def _compute_sales_count(self):
        ...
        done_states = self.env['sale_custom.sale_report']._get_done_states()
        ...
        for product, product_uom_qty in self.env['sale_custom.sale_report']._read_group(domain, ['product_id'], ['product_uom_qty:sum']):
    ...
    def _compute_product_is_in_sale_order(self):
        ...
        read_group_data = self.env['sale_custom.order.line']._read_group(
    ...
    def _search_product_is_in_sale_order(self, operator, value):
        ...
        product_ids = self.env['sale_custom.order.line'].search([
    ...
    def _filter_to_unlink(self):
        ...
        lines = self.env['sale_custom.order.line']._read_group(domain, ['product_id'])
...
class ProductAttributeCustomValue(models.Model):
    ...
    sale_order_line_id = fields.Many2one('sale_custom.order.line', string="Sales Order Line", ondelete='cascade')
