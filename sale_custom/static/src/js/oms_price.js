...
function injectBasePrice(tiers, productId, fallbackBasePrice = 0) {
    const basePrice = getStoredShopPrice(productId) || fallbackBasePrice || getDisplayedUnitPriceFallback();
    ...
    log("injectBasePrice", { productId, basePrice, firstTier });
    return cloned;
}
...
function findUnitPriceNode() {
    ...
    const explicit =
        root.querySelector('.product_price .oe_price .oe_currency_value') ||
        root.querySelector('.oe_price .oe_currency_value');
    if (explicit && isVisible(explicit) && !isInsideQtyTable(explicit)) {
        return explicit;
    }

    const totalNode = findTotalPriceNode();
    const all = getVisibleMoneyNodes(root).filter((node) => {
        if (node === totalNode) return false;
        if (node.classList?.contains("oe_price") && node.querySelector(".oe_currency_value")) return false;
        return true;
    });
...
function getUnitSourceNodes() {
    ...
    nodes.push(...root.querySelectorAll(".product_price .oe_price .oe_currency_value, .oe_price .oe_currency_value, meta[itemprop='price'], [itemprop='price'][content]"));
    return uniqNodes(nodes.filter((node) => node && !isInsideQtyTable(node) && !isInTotalBlock(node)));
}

function getTotalSourceNodes() {
    const nodes = [findTotalPriceNode()];
    const labelEl = findTotalLabelElement();
    const row = findSummaryRow(labelEl);
    if (row) {
        nodes.push(...row.querySelectorAll(".oe_currency_value, .oe_price, [data-total-price], [data-price-total], span, strong, b, div, td, p"));
    }
    return uniqNodes(
        nodes.filter((node) => {
...
function syncPriceSourceAttributes(unit, total) {
    ...
    [root, form].filter(Boolean).forEach((node) => {
    ...
    root.querySelectorAll("meta[itemprop='price'], [itemprop='price'][content]").forEach((node) => {
        node.setAttribute("content", String(Math.round(unit)));
    });

    root.querySelectorAll("[data-price], [data-unit-price], [data-product-price], [data-list-price]").forEach((node) => {
        if (isInsideQtyTable(node) || isInTotalBlock(node)) return;
        setAttrIfPresent(node, ["data-price", "data-unit-price", "data-product-price", "data-list-price"], unit);
    });

    root.querySelectorAll("[data-total-price], [data-price-total]").forEach((node) => {
...
function setDisplayedAmounts(unitPrice, qty) {
    ...
    const unitNodes = getUnitSourceNodes();
    ...
    log("setDisplayedAmounts", { unit, qty, total, unitNodes, totalNodes });
}

async function loadTiers(productId, fallbackBasePrice = 0) {
    let rawTiers = [];
    try {
        rawTiers = await rpc("/oms/price_tiers", { product_id: productId });
    } catch (e) {
        err("RPC /oms/price_tiers failed", e);
        return [];
    }

    const effective = buildEffectiveTiers(rawTiers);
    const tiers = injectBasePrice(effective, productId, fallbackBasePrice);
    log("effective tiers", { rawTiers, tiers, fallbackBasePrice });
    return tiers;
}
...
function bindQtyEvents() {
    ...
    if (!state.docBound) {
        ...
        document.addEventListener("variant_changed", () => {
            state.lastSignature = null;
            scheduleApplySeries(true, [120, 320, 700, 1200]);
        });
        ...
        document.addEventListener(
            ...
            (ev) => {
                if (ev.target.closest("form.js_main_product")) {
                    state.lastSignature = null;
                    scheduleApplySeries(true, [100, 260, 620, 1200]);
                }
            },
            true
        );
...
async function bootOnce(forceReload = false) {
    ...
    const productId = n(wrap.getAttribute("data-product-id"), 0);
    if (!productId) {
    ...
    const needReload = forceReload || state.productId !== productId || !state.tiers.length || !tbody.children.length;

    if (needReload) {
        state.productId = productId;
        state.tiers = await loadTiers(productId, state.initialBasePrice);
        state.lastSignature = null;
