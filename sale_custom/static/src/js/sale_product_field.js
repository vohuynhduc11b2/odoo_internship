...
export class SaleOrderLineProductField extends ProductLabelSectionAndNoteField {
    static template = "sale_custom.SaleProductField";
    static props = {
    ...
    onClick(ev) {
        // Override to get internal link to products in AUT that cannot be edited
...
if (fields.contains?.("sol_product_many2one")) {
    fields.remove("sol_product_many2one");
}
fields.add("sol_product_many2one", saleOrderLineProductField);
