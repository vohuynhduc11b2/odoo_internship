...
def _synchronize_cron(env):
    send_invoice_cron = env.ref('sale_custom.send_invoice_cron', raise_if_not_found=False)
    if send_invoice_cron:
        config = env['ir.config_parameter'].get_param('sale_custom.automatic_invoice', False)
...
def post_init_hook_2(cr, registry):
    from odoo.tools.sql import create_index
    create_index(cr, 'sale_order_date_order_id_idx', 'sale_order', ["date_order desc", "id desc"])
