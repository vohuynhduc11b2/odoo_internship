...
class SaleReport(models.Model):
    ...
    # sale_custom.order.line fields
    order_reference = fields.Reference(
        string='Order',
        selection=[('sale_custom.order', 'Sales Order')],
    ...
    def _select_sale(self):
        select_ = f"""
            ...
            concat('sale_custom.order', ',', s.id) AS order_reference"""
    ...
    def _from_sale(self):
        ...
        return f"""
            sale_custom_order_line l
            LEFT JOIN sale_custom_order s ON s.id=l.order_id
