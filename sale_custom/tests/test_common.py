...
class TestSaleCommon(SaleCommon):
    ...
    def test_common(self):
        ...
        self.assertEqual(self.sale_order_custom.partner_id, self.partner)
        self.assertEqual(self.sale_order_custom.partner_invoice_id, self.partner)
        self.assertEqual(self.sale_order_custom.partner_shipping_id, self.partner)
        self.assertEqual(self.sale_order_custom.pricelist_id, self.pricelist)
        self.assertEqual(self.sale_order_custom.currency_id.name, self.currency.name)
        self.assertEqual(self.sale_order_custom.team_id, self.sale_team)
        self.assertEqual(self.sale_order_custom.state, 'draft')

        consumable_line, service_line = self.sale_order_custom.order_line
        ...
        self.assertEqual(self.sale_order_custom.amount_total, 725.0)
