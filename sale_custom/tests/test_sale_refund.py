...
class TestSaleRefund(TestSaleCommon):
    ...
    def setUpClass(cls):
        ...
        payment = cls.env['sale_custom.advance.payment.inv'].with_context({
        ...
        cls.invoice = cls.sale_order_custom.invoice_ids[0]
    ...
    def test_refund_create(self):
        ...
        invoice_refund = self.sale_order_custom.invoice_ids.sorted(key=lambda inv: inv.id, reverse=False)[-1]  # the first invoice, its refund, and the new invoice
        ...
        self.assertEqual(self.sale_order_custom.invoice_count, 2, "The SO should have 2 related invoices: the original, the new refund")
        self.assertEqual(len(self.sale_order_custom.invoice_ids.filtered(lambda inv: inv.move_type == 'out_refund')), 1, "The SO should be linked to only one refund")
        self.assertEqual(len(self.sale_order_custom.invoice_ids.filtered(lambda inv: inv.move_type == 'out_invoice')), 1, "The SO should be linked to only one customer invoices")
    ...
    def test_refund_modify(self):
        ...
        self.assertEqual(invoice_refund.state, 'draft', 'Last Customer invoices should be in draft')
        self.assertEqual(self.sale_order_custom.invoice_count, 3, "The SO should have 3 related invoices: the original, the refund, and the new one")
        self.assertEqual(len(self.sale_order_custom.invoice_ids.filtered(lambda inv: inv.move_type == 'out_refund')), 1, "The SO should be linked to only one refund")
        self.assertEqual(len(self.sale_order_custom.invoice_ids.filtered(lambda inv: inv.move_type == 'out_invoice')), 2, "The SO should be linked to two customer invoices")
    ...
    def test_refund_invoice_with_downpayment(self):
        sale_order_refund = self.env['sale_custom.order'].create({
            'partner_id': self.partner_a.id,
            'partner_invoice_id': self.partner_a.id,
            'partner_shipping_id': self.partner_a.id,
            'pricelist_id': self.company_data['default_pricelist'].id,
        })
        sol_product = self.env['sale_custom.order.line'].create({
        ...
        downpayment = self.env['sale_custom.advance.payment.inv'].with_context(so_context).create({
        ...
        payment = self.env['sale_custom.advance.payment.inv'].with_context(so_context).create({})
        payment.create_invoices()
