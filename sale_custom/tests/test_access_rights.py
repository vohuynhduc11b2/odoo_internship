...
class TestAccessRights(BaseUsersCommon, SaleCommon, MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.sale_order_custom.user_id = cls.sale_user

    def test_access_sales_manager(self):
        """ Test sales manager's access rights """
        SaleOrder = self.env['sale_custom.order'].with_user(self.sale_manager)
        so_as_sale_manager = SaleOrder.browse(self.sale_order_custom.id)
        ...
        self.assertIn(
            sale_order_custom.id, SaleOrder.search([]).ids,
    ...
    def test_access_sales_person(self):
        ...
        SaleOrder = self.env['sale_custom.order'].with_user(self.sale_user2)
        so_as_salesperson = SaleOrder.browse(self.sale_order_custom.id)
        ...
        self.sale_order_custom.write({'user_id': self.sale_user2.id})
        ...
        with self.assertRaises(AccessError):
            self.env['sale_custom.order'].with_user(self.sale_user2).create({
                'partner_id': self.partner.id,
                'user_id': self.sale_user.id
            })
    ...
    def test_access_portal_user(self):
        ...
        SaleOrder = self.env['sale_custom.order'].with_user(self.user_portal)
        so_as_portal_user = SaleOrder.browse(self.sale_order_custom.id)

        # Portal user can see the confirmed SO for which they are assigned as a customer
        with self.assertRaises(AccessError):
            so_as_portal_user.read()

        self.sale_order_custom.partner_id = self.user_portal.partner_id
        ...
        self.sale_order_custom.action_cancel()
        with self.assertRaises(AccessError):
    ...
    def test_access_employee(self):
        ...
        SaleOrder = self.env['sale_custom.order'].with_user(self.user_internal)
        so_as_internal_user = SaleOrder.browse(self.sale_order_custom.id)
