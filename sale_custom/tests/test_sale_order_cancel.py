...
class TestSaleOrderCancel(SaleCommon, TestSalesCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.template = cls.env.ref('sale_custom.mail_template_sale_cancellation')
        cls.template.write({
        ...
        cls.manager_order, cls.salesman_order = cls.env['sale_custom.order'].create([
    ...
    def test_salesman_record_rules(self):
        cancel = self.env['sale_custom.order.cancel'].create({
