...
class TestSaleReportCurrencyRate(SaleCommon):
    ...
    def test_sale_report_foreign_currency(self):
        ...
        report_lines = self.env['sale_custom.sale_report'].sudo().with_context(
            allow_company_ids=[self.usd_cmp.id, self.eur_cmp.id]
        ).search([('order_reference', 'in', [f'sale_custom.order,{so_id}' for so_id in sale_orders.ids])])
    ...
    def test_sale_report_with_downpayment(self):
        ...
        downpayment = self.env['sale_custom.advance.payment.inv'].with_context(active_ids=order.ids).create({
        ...
        amount_line = self.env['sale_custom.sale_report'].read_group(
            [('order_reference', '=', f'sale_custom.order,{order.id}')],
