...
class TestSaleOrder(SaleCommon):
    ...
    def test_computes_auto_fill(self):
        ...
        order = self.env['sale_custom.order'].create({
    ...
    def test_sale_order_standard_flow(self):
        self.assertEqual(self.sale_order_custom.amount_total, 725.0, 'Sale: total amount is wrong')
    ...
    def test_sale_order_send_to_self(self):
        ...
        sale_order = self.env['sale_custom.order'].with_user(self.sale_user).create({
            'partner_id': self.sale_user.partner_id.id,
        })
        email_ctx = sale_order_custom.action_quotation_send().get('context', {})
        ...
        sale_order_custom.with_context(**email_ctx).with_user(self.sale_user).message_post_with_source(
            mail_template,
            subtype_xmlid='mail.mt_comment',
        )
        self.assertTrue(sale_order_custom.state == 'sent', 'Sale : state should be changed to sent')
        mail_message = sale_order_custom.message_ids[0]
        self.assertEqual(mail_message.author_id, sale_order_custom.partner_id, 'Sale: author should be same as customer')
    ...
    def test_sale_sequence(self):
        self.env['ir.sequence'].search([
            ('code', '=', 'sale_custom.order'),
        ]).write({
            'use_date_range': True, 'prefix': 'SO/%(range_year)s/',
        })
        sale_order = self.sale_order_custom.copy({'date_order': '2019-01-01'})
        self.assertTrue(sale_order_custom.name.startswith('SO/2019/'))
        sale_order = self.sale_order_custom.copy({'date_order': '2020-01-01'})
        self.assertTrue(sale_order_custom.name.startswith('SO/2020/'))
        # In EU/BXL tz, this is actually already 01/01/2020
        sale_order = self.sale_order_custom.with_context(tz='Europe/Brussels').copy({'date_order': '2019-12-31 23:30:00'})
        self.assertTrue(sale_order_custom.name.startswith('SO/2020/'))
    ...
    def test_unlink_cancel(self):
        ...
        so_copy = self.sale_order_custom.copy()
        ...
        self.sale_order_custom.action_confirm()
        self.assertTrue(self.sale_order_custom.state == 'sale', 'Sale: SO should be in state "sale"')
        with self.assertRaises(UserError):
            self.sale_order_custom.unlink()

        self.sale_order_custom.action_lock()
        self.assertTrue(self.sale_order_custom.state == 'sale')
        self.assertTrue(self.sale_order_custom.locked)
        with self.assertRaises(UserError):
            self.sale_order_custom.unlink()
    ...
    def test_compute_packaging_00(self):
        ...
        so2 = self.env['sale_custom.order'].create({
            'partner_id': self.partner.id,
        })
        so2_form = Form(so2)
    ...
    def test_compute_packaging_01(self):
        ...
        so2 = self.env['sale_custom.order'].with_company(company2).create({
            'partner_id': self.partner.id,
        })
        so2_form = Form(so2)
        with so2_form.order_line.new() as line:
    ...
    def _create_sale_order(self):
        ...
        return self.env['sale_custom.order'].with_context(
    ...
    def test_invoicing_terms(self):
        ...
        self.assertEqual(sale_order_custom.note, "<p>Coin coin</p>")

        # Html invoice terms (/terms page)
        self.env.company.terms_type = 'html'
        sale_order = self._create_sale_order()
        self.assertTrue(sale_order_custom.note.startswith("<p>Terms &amp; Conditions: "))
    ...
    def test_validity_days(self):
        ...
        with freeze_time("2020-05-02"):
            ...
            self.assertEqual(sale_order_custom.validity_date, fields.Date.today() + timedelta(days=5))
        self.env.company.quotation_validity_days = 0
        sale_order = self._create_sale_order()
        self.assertFalse(
            sale_order_custom.validity_date,
    ...
    def test_so_names(self):
        ...
        """
        SaleOrder = self.env['sale_custom.order'].with_context(sale_show_partner_name=True)

        res = SaleOrder.name_search(name=self.sale_order_custom.partner_id.name)
        self.assertEqual(res[0][0], self.sale_order_custom.id)

        self.assertNotIn(self.sale_order_custom.partner_id.name, self.sale_order_custom.display_name)
        self.assertIn(
            self.sale_order_custom.partner_id.name,
            self.sale_order_custom.with_context(sale_show_partner_name=True).display_name)

    def test_sol_names(self):
        """Check that the SOL description gets used for the display name."""
        self.sale_order_custom.order_line = [
            Command.create({'is_downpayment': True}),
            Command.create({'display_type': 'line_note', 'name': "Foo\nBar\nBaz"}),
        ]
        sol1, sol2, sol3, sol4 = self.sale_order_custom.order_line
        sol1.name += "\nOK THANK YOU\nGOOD BYE"

        self.assertEqual(
            sol1.display_name,
            f"{self.sale_order_custom.name} - OK THANK YOU ({self.partner.name})",
            "Product line with description should display the first line of description",
        )
        self.assertEqual(
            sol2.display_name,
            f"{self.sale_order_custom.name} - {sol2.product_id.name} ({self.partner.name})",
            "Product line without description should display the product name",
        )
        self.assertEqual(
            sol3.display_name,
            f"{self.sale_order_custom.name} - {sol3.name} ({self.partner.name})",
            "Down payment line should display the down payment name",
        )
        self.assertEqual(
            sol4.display_name,
            f"{self.sale_order_custom.name} - Foo ({self.partner.name})",
            "Multi-line note should display the first line only",
        )

    def test_state_changes(self):
        """Test some untested state changes methods & logic."""
        self.sale_order_custom.action_quotation_sent()

        self.assertEqual(self.sale_order_custom.state, 'sent')
        self.assertIn(self.sale_order_custom.partner_id, self.sale_order_custom.message_follower_ids.partner_id)

        self.env.user.groups_id += self.env.ref('sale_custom.group_auto_done_setting')
        self.sale_order_custom.action_confirm()
        self.assertEqual(self.sale_order_custom.state, 'sale')
        self.assertTrue(self.sale_order_custom.locked)
        with self.assertRaises(UserError):
            self.sale_order_custom.action_confirm()

        self.sale_order_custom.action_unlock()
        self.assertEqual(self.sale_order_custom.state, 'sale')

    def test_sol_name_search(self):
        # Shouldn't raise
        self.env['sale_custom.order']._search([('order_line', 'ilike', 'product')])

        name_search_data = self.env['sale_custom.order.line'].name_search(name=self.sale_order_custom.name)
        sol_ids_found = dict(name_search_data).keys()
        self.assertEqual(list(sol_ids_found), self.sale_order_custom.order_line.ids)
    ...
    def test_zero_quantity(self):
        ...
        """
        order_line = self.sale_order_custom.order_line[0]
    ...
    def test_discount_rounding(self):
        ...
        self.assertEqual(sale_order_custom.order_line.price_subtotal, 49.44, "Subtotal should be equal to 192 * (1 - 0.7425)")
        self.assertEqual(sale_order_custom.order_line.discount, 74.25)
    ...
    def test_tax_amount_rounding(self):
        ...
        self.assertEqual(sale_order_custom.amount_total, 15.42, "")
        ...
        self.assertEqual(sale_order_custom.amount_total, 15.41, "")

    def test_order_auto_lock_with_public_user(self):
        public_user = self.env.ref('base.public_user')
        self.sale_order_custom.create_uid.groups_id += self.env.ref('sale_custom.group_auto_done_setting')
        self.sale_order_custom.with_user(public_user.id).sudo().action_confirm()

        self.assertFalse(public_user.has_group('sale_custom.group_auto_done_setting'))
        self.assertTrue(self.sale_order_custom.locked)
    ...
    def test_draft_quotation_followers(self):
        ...
        sale_order_custom.partner_id = self.partner2

        self.assertNotIn(self.partner2, sale_order_custom.message_partner_ids)

    def test_sent_quotation_followers(self):
        sale_order = self.env['sale_custom.order'].create({
            'partner_id': self.partner1.id,
        })
        sale_order_custom.action_quotation_sent()

        sale_order_custom.partner_id = self.partner2

        self.assertIn(self.partner2, sale_order_custom.message_partner_ids)

    def test_so_discount_is_not_reset(self):
        """ Discounts should not be recomputed on order confirmation """
        with patch(
            'odoo.addons.sale.models.sale_order_line_custom.SaleOrderLine'
            '._compute_discount'
        ) as patched:
            self.sale_order_custom.action_confirm()
            self.sale_order_custom.order_line.flush_recordset(['discount'])
    ...
    def test_so_is_not_invoiceable_if_only_discount_line_is_to_invoice(self):
        self.sale_order_custom.order_line.product_id.invoice_policy = 'delivery'
        self.sale_order_custom.action_confirm()

        self.assertEqual(self.sale_order_custom.invoice_status, 'no')
        standard_lines = self.sale_order_custom.order_line

        self.env['sale_custom.order.discount'].create({
            'sale_order_id': self.sale_order_custom.id,
            'discount_amount': 33,
            'discount_type': 'amount',
        }).action_apply_discount()

        # Only the discount line is invoiceable (there are lines not invoiced and not invoiceable)
        discount_line = self.sale_order_custom.order_line - standard_lines
    ...
    def test_so_is_invoiceable_if_only_discount_line_remains_to_invoice(self):
        ...
        self.sale_order_custom._create_invoices()

        self.assertEqual(self.sale_order_custom.invoice_status, 'invoiced')
        ...
        discount_line = (self.sale_order_custom.order_line - standard_lines)
        self.assertEqual(discount_line.invoice_status, 'to invoice')
        self.assertEqual(self.sale_order_custom.invoice_status, 'to invoice')
    ...
    def test_sale_order_line_product_taxes_on_branch(self):
        ...
        so_form = Form(self.env['sale_custom.order'].with_company(branch_xx))
    ...
    def test_price_recomputation_on_readonly_unit_price(self):
        ...
        # Order update
        product_sol = self.sale_order_custom.order_line[0]
        ...
        # Order creation
        new_order = self.env['sale_custom.order'].create({
            'partner_id': self.partner.id,
            'order_line': [
                Command.create({
                    'product_id': self.product.id,
...
class TestSaleOrderInvoicing(AccountTestInvoicingCommon, SaleCommon):
    def test_invoice_state_when_ordered_quantity_is_negative(self):
        ...
        sale_order_custom._create_invoices(final=True)
        self.assertTrue(sale_order_custom.invoice_status == 'invoiced', 'Sale: The invoicing status of the SO should be "invoiced"')
...
class TestSalesTeam(SaleCommon):
    ...
    def test_assign_sales_team_from_partner_user(self):
        ...
        self.assertEqual(sale_order_custom.team_id.id, self.sale_team.id, 'Should assign to team of sales person')
    ...
    def test_assign_sales_team_when_changing_user(self):
        ...
        sale_order_custom.user_id = self.user_in_team
        self.assertEqual(sale_order_custom.team_id.id, self.sale_team.id, 'Should assign to team of sales person')
    ...
    def test_keep_sales_team_when_changing_user_with_no_team(self):
        ...
        sale_order_custom.user_id = self.user_not_in_team
        self.assertEqual(sale_order_custom.team_id.id, self.sale_team_2.id, 'Should not reset the team to default')
    ...
    def test_sale_order_analytic_distribution_change(self):
        ...
        so_no_analytic_account = self.env['sale_custom.order'].create({
            'partner_id': partner.id,
        })
        sol_no_analytic_account = self.env['sale_custom.order.line'].create({
            'name': super_product.name,
            'product_id': super_product.id,
        ...
        sale_order_custom.action_confirm()
        sol_on_confirmed_order = self.env['sale_custom.order.line'].create({
            'name': super_product.name,
            'product_id': super_product.id,
            'order_id': sale_order_custom.id,
        })
    ...
    def test_cannot_assign_tax_of_mismatch_company(self):
        ...
        sol = self.env['sale_custom.order.line'].sudo().create({
            'name': product.name,
    ...
    def test_assign_tax_multi_company(self):
        ...
        sale_order = self.env['sale_custom.order'].create({'partner_id': self.partner.id, 'company_id': root_company.child_ids[0].id})
        product = self.env['product.product'].create({'name': 'Product'})

        # In sudo to simulate an user that have access to both companies.
        sol_b1 = self.env['sale_custom.order.line'].sudo().create({
            'name': product.name,
            'product_id': product.id,
            'order_id': sale_order_custom.id,
            'tax_id': tax_b1,
    ...
    def test_downpayment_amount_constraints(self):
        ...
        self.sale_order_custom.require_payment = True
        with self.assertRaises(ValidationError):
            self.sale_order_custom.prepayment_percent = -1
        with self.assertRaises(ValidationError):
            self.sale_order_custom.prepayment_percent = 1.01
    ...
    def test_qty_delivered_on_creation(self):
        ...
        self.assertEqual(self.env['sale_custom.order.line'].search(['&', ('order_id', '=', sale_order_custom.id), ('qty_delivered', '=', 0.0)]), sale_order_custom.order_line)
...
class TestSaleMailComposerUI(MailCommon, HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.quotation = cls.env['sale_custom.order'].create({
