...
class TestSalePayment(AccountPaymentCommon, SaleCommon, PaymentHttpCommon):
    ...
    def setUpClass(cls):
        ...
        cls.currency = cls.sale_order_custom.currency_id
        cls.partner = cls.sale_order_custom.partner_invoice_id
    ...
    def test_11_so_payment_link(self):
        ...
        self.amount = self.sale_order_custom.amount_total
        route_values = self._prepare_pay_values()
        route_values['sale_order_id'] = self.sale_order_custom.id
        ...
        self.assertEqual(tx_context['currency_id'], self.sale_order_custom.currency_id.id)
        self.assertEqual(tx_context['partner_id'], self.sale_order_custom.partner_invoice_id.id)
        self.assertEqual(tx_context['amount'], self.sale_order_custom.amount_total)
        ...
        self.assertEqual(tx_sudo.partner_id, self.sale_order_custom.partner_invoice_id)
        self.assertEqual(tx_sudo.company_id, self.sale_order_custom.company_id)
        self.assertEqual(tx_sudo.currency_id, self.sale_order_custom.currency_id)
        self.assertEqual(tx_sudo.reference, self.sale_order_custom.name)

        # Check validation of transaction correctly confirms the SO
        self.assertEqual(self.sale_order_custom.state, 'draft')
    ...
    def test_so_payment_link_with_different_partner_invoice(self):
        ...
        self.sale_order_custom.partner_invoice_id = self.portal_partner
        self.partner = self.sale_order_custom.partner_invoice_id
        route_values = self._prepare_pay_values()
        route_values['sale_order_id'] = self.sale_order_custom.id
    ...
    def test_12_so_partial_payment_link(self):
        ...
        self.amount = self.sale_order_custom.amount_total / 2.0
        pay_route_values = self._prepare_pay_values()
        pay_route_values['sale_order_id'] = self.sale_order_custom.id
        ...
        self.assertTrue(self.sale_order_custom._has_to_be_paid())
        with mute_logger('odoo.addons.sale.models.payment_transaction'):
            tx_sudo._post_process()
        self.assertEqual(self.sale_order_custom.state, 'draft') # Only a partial amount was paid
        ...
        self.assertEqual(tx2_sudo.partner_id, self.sale_order_custom.partner_invoice_id)
        self.assertEqual(tx2_sudo.company_id, self.sale_order_custom.company_id)
        self.assertEqual(tx2_sudo.currency_id, self.sale_order_custom.currency_id)

        self.assertEqual(self.sale_order_custom.state, 'draft')
        self.assertEqual(self.sale_order_custom.transaction_ids, tx_sudo + tx2_sudo)
    ...
    def test_13_sale_automatic_partial_payment_link_delivery(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param('sale_custom.automatic_invoice', 'True')
        ...
        self.assertEqual(self.sale_order_custom.state, 'draft', 'a partial transaction with automatic invoice and invoice_policy = delivery should not validate a quote')
    ...
    def test_confirmed_transactions_comfirms_so_with_multiple_transaction(self):
        ...
        self._create_transaction(
            flow='redirect',
            sale_order_ids=[self.sale_order_custom.id],
            state='draft',
        ...
        tx = self._create_transaction(flow='redirect', sale_order_ids=[self.sale_order_custom.id], state='done')
        tx._post_process()
    ...
    def test_auto_confirm_and_auto_invoice(self):
        ...
        self.assertTrue(self.sale_order_custom.invoice_ids)
    ...
    def test_so_partial_payment_no_invoice(self):
        ...
        # Create the payment
        self.amount = self.sale_order_custom.amount_total / 10.
        tx = self._create_transaction(flow='redirect', sale_order_ids=[self.sale_order_custom.id], state='done')
        with mute_logger('odoo.addons.sale.models.payment_transaction'):
            tx._post_process()
        ...
        self.assertFalse(self.sale_order_custom.invoice_ids)
    ...
    def test_invoice_is_final(self):
        ...
        with mute_logger('odoo.addons.sale.models.payment_transaction'), patch(
            'odoo.addons.sale.models.sale_order_custom.SaleOrder._create_invoices',
    ...
    def test_linked_transactions_when_invoicing(self):
        ...
        partial_amount = self.sale_order_custom.amount_total - 2
        ...
        self._create_transaction(
            flow='direct', sale_order_ids=[self.sale_order_custom.id], state='draft', reference='draft_tx'
        )
        self._create_transaction(
            flow='direct', sale_order_ids=[self.sale_order_custom.id], state='error', reference='error_tx'
        )
        self._create_transaction(
            flow='direct', sale_order_ids=[self.sale_order_custom.id], state='cancel', reference='cncl_tx'
        )

        msg = "The sale order should be linked to 5 transactions."
        self.assertEqual(len(self.sale_order_custom.transaction_ids), 5, msg=msg)

        self.sale_order_custom.action_confirm()
        self.sale_order_custom._create_invoices()

        self.assertEqual(len(self.sale_order_custom.invoice_ids), 1, msg="1 invoice should be created.")
        ...
        self.sale_order_custom.order_line[0].product_uom_qty += 2
        self.sale_order_custom._create_invoices()

        second_invoice = self.sale_order_custom.invoice_ids - first_invoice
    ...
    def test_downpayment_confirm_sale_order_sufficient_amount(self):
        ...
        self.sale_order_custom.require_payment = True
        self.sale_order_custom.prepayment_percent = 0.1
        order_amount = self.sale_order_custom.amount_total

        tx = self._create_transaction(
            flow='direct',
            amount=order_amount * self.sale_order_custom.prepayment_percent,
            sale_order_ids=[self.sale_order_custom.id],
            state='done',
        )
        with mute_logger('odoo.addons.sale.models.payment_transaction'):
    ...
    def test_downpayment_confirm_sale_order_insufficient_amount(self):
        ...
        self.sale_order_custom.require_payment = True
        self.sale_order_custom.prepayment_percent = 0.2
        order_amount = self.sale_order_custom.amount_total
        ...
        self.assertTrue(self.sale_order_custom.state == 'draft')
    ...
    def test_downpayment_automatic_invoice(self):
        ...
        tx = self._create_transaction(
            flow='direct',
            amount=self.sale_order_custom.amount_total * self.sale_order_custom.prepayment_percent,
            sale_order_ids=[self.sale_order_custom.id],
            state='done')
    ...
    def test_check_portal_access_token_before_rerouting_flow(self):
        ...
        with patch.object(CustomerPortal, '_document_check_access') as mock:
            ...
            payment_portal_controller._get_extra_payment_form_values(
                sale_order_id=self.sale_order_custom.id, access_token='whatever'
            )
            self.assertEqual(
    ...
    def test_transaction_route_rejects_unexpected_kwarg(self):
        url = self._build_url(f'/my/orders/{self.sale_order_custom.id}/transaction')
        route_kwargs = {
            'access_token': self.sale_order_custom._portal_ensure_token(),
    ...
    def test_partial_payment_confirm_order(self):
        ...
        self.amount = self.sale_order_custom.amount_total / 2

        with patch(
            'odoo.addons.sale.models.sale_order_custom.SaleOrder._send_order_notification_mail',
            ...
            notification_mail_mock.assert_called_once_with(
                self.env.ref('sale_custom.mail_template_sale_payment_executed'))
            self.assertEqual(self.sale_order_custom.state, 'draft')
            self.assertEqual(self.sale_order_custom.amount_paid, self.amount)
            ...
            notification_mail_mock.assert_called_with(
                self.env.ref('sale_custom.mail_template_sale_confirmation'))
            self.assertEqual(self.sale_order_custom.state, 'sale')
