...
class TestSaleOnchanges(TransactionCase):
    ...
    def test_sale_warnings(self):
        ...
        sale_order = self.env['sale_custom.order'].create({'partner_id': partner_with_warning.id})
        warning = sale_order_custom._onchange_partner_id_warning()
        ...
        sale_order_custom.partner_id = partner_with_block_warning
        warning = sale_order_custom._onchange_partner_id_warning()
        self.assertDictEqual(warning, {
            'warning': {
        ...
        self.assertFalse(sale_order_custom.partner_id.id)
        self.assertFalse(sale_order_custom.partner_invoice_id.id)
        self.assertFalse(sale_order_custom.partner_shipping_id.id)
        self.assertFalse(sale_order_custom.pricelist_id.id)

        # Reuse non blocking partner for product warning tests
        sale_order_custom.partner_id = partner_with_warning
        ...
        sale_order_line = self.env['sale_custom.order.line'].create({
            'order_id': sale_order_custom.id,
            'product_id': product_with_warning.id,
        })
        warning = sale_order_line_custom._onchange_product_id_warning()
        self.assertDictEqual(warning, {
            'warning': {
        ...
        sale_order_line_custom.product_id = product_with_block_warning
        warning = sale_order_line_custom._onchange_product_id_warning()
        ...
        self.assertFalse(sale_order_line_custom.product_id.id)
