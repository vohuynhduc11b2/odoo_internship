...
class TestSaleOrderDownPayment(TestSaleCommon):
    ...
    def setUpClass(cls):
        ...
        SaleOrder = cls.env['sale_custom.order']
        ...
        cls.sol_product_order = cls.env['sale_custom.order.line'].create({
            'name': cls.company_data['product_order_no'].name,
            'product_id': cls.company_data['product_order_no'].id,
            'product_uom_qty': 2,
            'product_uom': cls.company_data['product_order_no'].uom_id.id,
            'price_unit': 100,
            'order_id': cls.sale_order_custom.id,
            'tax_id': False,
        })
        cls.sol_serv_deliver = cls.env['sale_custom.order.line'].create({
        ...
        cls.sol_serv_order = cls.env['sale_custom.order.line'].create({
        ...
        cls.sol_product_deliver = cls.env['sale_custom.order.line'].create({
    ...
    def make_downpayment(cls, **kwargs):
        ...
        downpayment = cls.env['sale_custom.advance.payment.inv'].with_context(so_context).create(payment_params)
        downpayment.create_invoices()
        if cls.sale_order_custom.state == 'draft':
            cls.sale_order_custom.action_confirm()
    ...
    def test_tax_and_account_breakdown(self):
        ...
        self.sale_order_custom.order_line[1].product_id.product_tmpl_id.property_account_income_id = income_acc_2

        self.sale_order_custom.order_line[0].tax_id = self.tax_15 + self.tax_10
        self.sale_order_custom.order_line[1].tax_id = self.tax_10
        self.sale_order_custom.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
        invoice = self.sale_order_custom.invoice_ids
        down_pay_amt = self.sale_order_custom.amount_total / 2
        ...
        self.sale_order_custom.order_line[1].qty_delivered = 2
        self.sale_order_custom.order_line[3].qty_delivered = 2

        # Full Invoice
        invoicing_wizard = self.env['sale_custom.advance.payment.inv'].create({
            'sale_order_ids': [Command.link(self.sale_order_custom.id)],
    ...
    def test_tax_with_diff_tax_on_invoice_breakdown(self):
        ...
        self.sale_order_custom.order_line[0].tax_id = self.tax_15
        (self.sale_order_custom.order_line - self.sale_order_custom.order_line[0]).unlink()
        self.make_downpayment(amount=25)
        first_invoice = self.sale_order_custom.invoice_ids
        first_invoice.invoice_line_ids.tax_ids = None
        first_invoice.action_post()
        self.make_downpayment(amount=25)
        invoice = self.sale_order_custom.invoice_ids - first_invoice
        down_pay_amt = self.sale_order_custom.amount_total / 4
        # ruff: noqa: E202
        expected = [
            # keys
            ['account_id',               'tax_ids',               'balance',   'price_total'],
    ...
    def test_tax_breakdown_other_currency(self):
        self.sale_order_custom.currency_id = self.other_currency  # rate = 2.0
        self.sale_order_custom.order_line[0].tax_id = self.tax_15 + self.tax_10
        self.sale_order_custom.order_line[1].tax_id = self.tax_10
        self.sale_order_custom.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
    ...
    def test_tax_breakdown_fixed_payment_method_with_taxes_on_all_lines(self):
        ...
        self.sale_order_custom.order_line[1].tax_id = self.tax_10
        self.sale_order_custom.order_line[2].tax_id = self.tax_10
        self.sale_order_custom.order_line[3].tax_id = self.tax_10
        self.make_downpayment(advance_payment_method='fixed', fixed_amount=222.5, amount=0)
        invoice = self.sale_order_custom.invoice_ids
        down_pay_amt = 222.5
        # pylint: disable=C0326
    ...
    def test_tax_price_include_breakdown(self):
        ...
        self.sale_order_custom.order_line[0].tax_id = tax_10_incl + self.tax_10
        self.sale_order_custom.order_line[1].tax_id = self.tax_10
        self.sale_order_custom.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
        invoice = self.sale_order_custom.invoice_ids
    ...
    def test_tax_price_include_include_base_amount_breakdown(self):
        ...
        self.sale_order_custom.order_line[0].tax_id = tax_10_pi_ba + self.tax_10
        self.sale_order_custom.order_line[1].tax_id = self.tax_10
        self.sale_order_custom.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
        invoice = self.sale_order_custom.invoice_ids
    ...
    def test_tax_breakdown_with_discount(self):
        self.sale_order_custom.order_line[0].tax_id = self.tax_10
        self.sale_order_custom.order_line[1].tax_id = self.tax_10
        self.sale_order_custom.order_line[1].discount = 25.0
        self.sale_order_custom.order_line[2].tax_id = self.tax_15
        self.make_downpayment()
        invoice = self.sale_order_custom.invoice_ids
        down_pay_amt = self.sale_order_custom.amount_total / 2
        # pylint: disable=C0326
    ...
    def test_tax_price_include_include_base_amount_breakdown_with_discount(self):
        ...
        self.tax_10.sequence = 2
        self.sale_order_custom.order_line[0].tax_id = tax_10_pi_ba + self.tax_10
        self.sale_order_custom.order_line[0].discount = 25.0
        self.sale_order_custom.order_line[1].tax_id = self.tax_10
        self.sale_order_custom.order_line[2].tax_id = self.tax_10
        self.make_downpayment()
        invoice = self.sale_order_custom.invoice_ids
    ...
    def test_tax_fixed_amount_breakdown(self):
        ...
        self.sale_order_custom.order_line[0].tax_id = tax_group_1
        self.sale_order_custom.order_line[1].tax_id = tax_group_2
        self.sale_order_custom.order_line[2].tax_id = tax_10_a
        self.make_downpayment()
    ...
    def test_tax_fixed_amount_price_include(self):
        ...
        downpayment = self.env['sale_custom.advance.payment.inv']\
            .with_context(active_ids=sale_order_custom.ids, active_model=sale_order_custom._name)\
            .create({
    ...
    def test_analytic_distribution(self):
        ...
        self.sale_order_custom.order_line[0].tax_id = self.tax_15 + self.tax_10
        self.sale_order_custom.order_line[0].analytic_distribution = {an_acc_01: 100}
        self.sale_order_custom.order_line[1].tax_id = self.tax_10
        self.sale_order_custom.order_line[1].analytic_distribution = {an_acc_01: 50, an_acc_02: 50}
        self.sale_order_custom.order_line[2].tax_id = self.tax_10
        self.sale_order_custom.order_line[2].analytic_distribution = {an_acc_01: 100}
        self.make_downpayment()
        invoice = self.sale_order_custom.invoice_ids
        down_pay_amt = self.sale_order_custom.amount_total / 2
        # pylint: disable=C0326
    ...
    def test_analytic_distribution_zero_line(self):
        ...
        an_acc_02 = str(self.env['account.analytic.account'].create({'name': 'Account 02', 'plan_id': analytic_plan.id}).id)
        self.sale_order_custom.order_line[0].tax_id = self.tax_15
        self.sale_order_custom.order_line[0].analytic_distribution = {an_acc_01: 50, an_acc_02: 50}
        self.sale_order_custom.order_line[1].tax_id = self.tax_10
        self.sale_order_custom.order_line[1].analytic_distribution = {an_acc_01: 50, an_acc_02: 50}
        self.sale_order_custom.order_line[2].tax_id = self.tax_10
        self.sale_order_custom.order_line[2].analytic_distribution = {an_acc_01: 50, an_acc_02: 50}
        self.sale_order_custom.order_line[2].price_unit = - self.sale_order_custom.order_line[1].price_unit
        self.make_downpayment()
        invoice = self.sale_order_custom.invoice_ids
        down_pay_amt = self.sale_order_custom.amount_total / 2
        # ruff: noqa: E271, E272
    ...
    def test_tax_price_include_amount_rounding(self):
        ...
        self.sale_order_custom.order_line[0].price_unit = 900
        self.sale_order_custom.order_line[0].product_uom_qty = 1
        self.sale_order_custom.order_line[0].tax_id = tax_21

        self.sale_order_custom.order_line[1].price_unit = 90
        self.sale_order_custom.order_line[1].product_uom_qty = 2
        self.sale_order_custom.order_line[1].tax_id = tax_21

        self.sale_order_custom.order_line[2].price_unit = 49
        self.sale_order_custom.order_line[2].product_uom_qty = 4
        self.sale_order_custom.order_line[2].tax_id = tax_21

        self.sale_order_custom.order_line[3].unlink()
        self.sale_order_custom.action_confirm()

        so_context = {
            'active_model': 'sale_custom.order',
            'active_ids': [self.sale_order_custom.id],
            'active_id': self.sale_order_custom.id,
        ...
        downpayment = self.env['sale_custom.advance.payment.inv'].with_context(so_context).create(payment_params)
        downpayment.create_invoices()
        invoice = self.sale_order_custom.invoice_ids
        # pylint: disable=C0326
        expected = [
    ...
    def test_tax_price_include_negative_amount_rounding_final_invoice(self):
        ...
        self.sale_order_custom.order_line[0].product_id = self.company_data['product_delivery_no'].id,
        self.sale_order_custom.order_line[0].product_uom_qty = 1
        self.sale_order_custom.order_line[0].qty_delivered = 0
        self.sale_order_custom.order_line[0].tax_id = tax_21_a
        self.sale_order_custom.order_line[0].price_unit = 1000

        self.sale_order_custom.order_line[1].product_id = self.company_data['product_delivery_no'].id,
        self.sale_order_custom.order_line[1].product_uom_qty = 1
        self.sale_order_custom.order_line[1].qty_delivered = 0
        self.sale_order_custom.order_line[1].tax_id = tax_21_b
        self.sale_order_custom.order_line[1].price_unit = 1000

        self.sale_order_custom.order_line[2:].unlink()
        ...
        downpayment = self.env['sale_custom.advance.payment.inv'].with_context({**so_context, 'raise_if_nothing_to_invoice': False}).create(payment_params)
        action = downpayment.create_invoices()
        invoice = self.env['account.move'].browse(action['res_id'])
        # pylint: disable=C0326
        expected = [
        ...
        invoice.unlink()
        self.sale_order_custom.order_line[0].qty_delivered = 1
        self.sale_order_custom.order_line[1].qty_delivered = 1
        downpayment = self.env['sale_custom.advance.payment.inv'].with_context(so_context).create(payment_params)
        action = downpayment.create_invoices()
        invoice = self.env['account.move'].browse(action['res_id'])
        # pylint: disable=C0326
    ...
    def test_tax_price_include_positive_amount_rounding_final_invoice(self):
        ...
        self.sale_order_custom.order_line[0].product_uom_qty = 1
        self.sale_order_custom.order_line[0].qty_delivered = 0
        self.sale_order_custom.order_line[0].tax_id = tax_24_a
        self.sale_order_custom.order_line[0].price_unit = 1000

        self.sale_order_custom.order_line[1].product_id = self.company_data['product_delivery_no'].id,
        self.sale_order_custom.order_line[1].product_uom_qty = 1
        self.sale_order_custom.order_line[1].qty_delivered = 0
        self.sale_order_custom.order_line[1].tax_id = tax_24_b
        self.sale_order_custom.order_line[1].price_unit = 1000
    ...
    def test_tax_price_include_small_amount_rounding_final_invoice(self):
        ...
        self.sale_order_custom.order_line[2].product_id = self.company_data['product_delivery_no'].id,
        self.sale_order_custom.order_line[2].product_uom_qty = 1
        self.sale_order_custom.order_line[2].qty_delivered = 1
        self.sale_order_custom.order_line[2].tax_id = tax_25_a
        self.sale_order_custom.order_line[2].price_unit = 968

        self.sale_order_custom.order_line[3].product_id = self.company_data['product_delivery_no'].id,
        self.sale_order_custom.order_line[3].product_uom_qty = 1
        self.sale_order_custom.order_line[3].qty_delivered = 1
        self.sale_order_custom.order_line[3].tax_id = tax_25_b
        self.sale_order_custom.order_line[3].price_unit = 968

        self.sale_order_custom.order_line[3].copy({
            'order_id':self.sale_order_custom.id,
            'tax_id': tax_25_c,
            'qty_delivered': 1,
        })

        self.sale_order_custom.order_line.qty_delivered_method = 'manual'
    ...
    def test_so_with_multiple_line_rounding(self):
        ...
        for i, price_unit in enumerate((10000, 10000, 10000, 50)):
            self.sale_order_custom.order_line[i].product_id = self.company_data['product_delivery_no'].id
            self.sale_order_custom.order_line[i].product_uom_qty = 1
            self.sale_order_custom.order_line[i].qty_delivered = 1
            self.sale_order_custom.order_line[i].tax_id = tax_20
            self.sale_order_custom.order_line[i].price_unit = price_unit
    ...
    def test_so_downpayment_invoice_credited_reinvoiced(self):
        ...
        invoice_to_be_refund = self.env['sale_custom.advance.payment.inv'].with_context(so_context).create(payment_params)
        ...
        final_invoice = self.env['sale_custom.advance.payment.inv'].with_context(so_context).create(payment_params)
    ...
    def test_downpayment_description(self):
        ...
        invoicing_wizard = self.env['sale_custom.advance.payment.inv'].create({
            'advance_payment_method': 'fixed',
            'fixed_amount': sale_order_custom.amount_total / 2.0,
            'sale_order_ids': [Command.link(sale_order_custom.id)],
        })

        # Down payment invoice
        action = invoicing_wizard.create_invoices()
        so_dp_line = sale_order_custom.order_line.filtered(
        ...
        self.assertEqual(sale_order_custom.invoice_status, 'to invoice')
        action = invoicing_wizard.create_invoices()
        full_invoice = self.env['account.move'].browse(action['res_id'])
