...
class TestSaleToInvoice(TestSaleCommon):
    ...
    def setUpClass(cls):
        ...
        cls.sale_order = cls.env['sale_custom.order'].create({
        ...
        ) = cls.sale_order_custom.order_line

        # Context
        cls.context = {
            'active_model': 'sale_custom.order',
            'active_ids': [cls.sale_order_custom.id],
            'active_id': cls.sale_order_custom.id,
    ...
    def _check_order_search(self, orders, domain, expected_result):
        ...
        result = self.env['sale_custom.order'].search(domain)
    ...
    def test_search_invoice_ids(self):
        ...
        self.sale_order_custom.action_confirm()

        # Tests before creating an invoice
        self._check_order_search(self.sale_order, [('invoice_ids', '=', False)], self.sale_order)
        self._check_order_search(self.sale_order, [('invoice_ids', '!=', False)], self.env['sale_custom.order'])

        # Create invoice
        moves = self.sale_order_custom._create_invoices()
    ...
    def test_downpayment(self):
        ...
        downpayment = self.env['sale_custom.advance.payment.inv'].with_context(self.context).create({
            'advance_payment_method': 'fixed',
            'fixed_amount': 50,
        })
        downpayment.create_invoices()
        downpayment2 = self.env['sale_custom.advance.payment.inv'].with_context(self.context).create({
            'advance_payment_method': 'fixed',
            'fixed_amount': 50,
        })
        ...
        self.assertEqual(len(self.sale_order_custom.invoice_ids), 2, 'Invoice should be created for the SO')
        downpayment_line = self.sale_order_custom.order_line.filtered(lambda l: l.is_downpayment and not l.display_type)
        ...
        payment = self.env['sale_custom.advance.payment.inv'].with_context(self.context).create({})
        payment.create_invoices()

        self.assertEqual(len(self.sale_order_custom.invoice_ids), 3, 'Invoice should be created for the SO')

        invoice = max(self.sale_order_custom.invoice_ids)
        self.assertEqual(len(invoice.invoice_line_ids.filtered(lambda l: not (l.display_type == 'line_section' and l.name == "Down Payments"))),
                         len(self.sale_order_custom.order_line.filtered(lambda l: not (l.display_type == 'line_section' and l.name == "Down Payments"))), 'All lines should be invoiced')
        ...
        self.assertEqual(invoice.amount_total, self.sale_order_custom.amount_total - sum(downpayment_line.mapped('price_unit')), 'Downpayment should be applied')
    ...
    def test_downpayment_validation(self):
        ...
        self.env.user.groups_id += self.env.ref('sale_custom.group_auto_done_setting')
        ...
        # Validate invoice
        self.sale_order_custom.invoice_ids.action_post()
    ...
    def test_downpayment_line_remains_on_SO(self):
        ...
        sale_order = self.env['sale_custom.order'].with_context(tracking_disable=True).create({
        ...
        # Confirm the SO
        sale_order_custom.action_confirm()
        # Update delivered quantity of SO line
        sale_order_custom.order_line.write({'qty_delivered': 5.0})
        context = {
            'active_model': 'sale_custom.order',
            'active_ids': [sale_order_custom.id],
            'active_id': sale_order_custom.id,
            'default_journal_id': self.company_data['default_journal_sale'].id,
        }
        # Let's do an invoice for a down payment of 50
        downpayment = self.env['sale_custom.advance.payment.inv'].with_context(context).create({
            'advance_payment_method': 'fixed',
            'fixed_amount': 50,
        })
        downpayment.create_invoices()
        # Let's do the invoice for the remaining amount
        payment = self.env['sale_custom.advance.payment.inv'].with_context(context).create({})
        payment.create_invoices()

        downpayment_line = sale_order_custom.order_line.filtered(lambda l: l.is_downpayment and not l.display_type)
        ...
        sale_order_custom.invoice_ids.action_post()
        self.assertEqual(downpayment_line[0].price_unit, 50, 'The down payment unit price should not change on SO')
    ...
    def test_downpayment_line_name(self):
        ...
        # Let's do an invoice for a down payment of 50
        self.env['sale_custom.advance.payment.inv'].with_context(context).create({
            'sale_order_ids': [Command.set(sale_order_custom.ids)],
            'advance_payment_method': 'fixed',
            'fixed_amount': 50,
        }).create_invoices()
        dp_line = sale_order_custom.order_line.filtered(
        ...
        # Confirm the invoice
        invoice = sale_order_custom.invoice_ids
        invoice.action_post()
    ...
    def test_downpayment_percentage_tax_icl(self):
        ...
        self.assertEqual(len(self.sale_order_custom.invoice_ids), 1, 'Invoice should be created for the SO')
        downpayment_line = self.sale_order_custom.order_line.filtered(lambda l: l.is_downpayment and not l.display_type)
        self.assertEqual(len(downpayment_line), 1, 'SO line downpayment should be created on SO')
        self.assertEqual(downpayment_line.price_unit, self.sale_order_custom.amount_total/2, 'downpayment should have the correct amount')

        invoice = self.sale_order_custom.invoice_ids[0]
        downpayment_aml = invoice.line_ids.filtered(lambda l: not (l.display_type == 'line_section' and l.name == "Down Payments"))[0]
        self.assertEqual(downpayment_aml.price_total, self.sale_order_custom.amount_total/2, 'downpayment should have the correct amount')
        self.assertEqual(downpayment_aml.price_unit, self.sale_order_custom.amount_total/2, 'downpayment should have the correct amount')
        invoice.action_post()
        self.assertEqual(downpayment_line.price_unit, self.sale_order_custom.amount_total/2, 'downpayment should have the correct amount')
    ...
    def test_invoice_with_discount(self):
        ...
        for line in self.sale_order_custom.order_line.filtered(lambda l: l.discount):
        ...
        # lines are in draft
        for line in self.sale_order_custom.order_line:
        ...
        for line, inv_line in zip(self.sale_order_custom.order_line, invoice.invoice_line_ids):
    ...
    def test_multiple_sale_orders_on_same_invoice(self):
        ...
        sale_order_data = self.sale_order_custom.copy_data()[0]
        ...
        self.sale_order_custom.create(sale_order_data)
    ...
    def test_invoice_with_sections(self):
        ...
        SaleOrderLine = self.env['sale_custom.order.line'].with_context(tracking_disable=True)
        SaleOrderLine.create({
            'name': 'Section',
            'display_type': 'line_section',
            'order_id': sale_order_custom.id,
        })
        ...
        invoice = sale_order_custom.invoice_ids[0]
    ...
    def test_invoice_combo_product(self):
        ...
        sale_order_custom.order_line = [Command.create({
            'product_id': product.id,
            'product_uom_qty': 3,
            'price_unit': 5.0,
            'tax_id': [],
            'combo_item_id': combo.combo_item_ids.id,
            'linked_line_id': sale_order_custom.order_line.id,
        }) for product, combo in zip(product_a + product_b, combo_a + combo_b)]

        # Confirm the SO
        sale_order_custom.action_confirm()

        self.assertEqual(sale_order_custom.order_line.mapped('qty_to_invoice'), [0.0, 0.0, 0.0])
        deliverables = sale_order_custom.order_line.filtered(
            lambda sol: sol.product_id.invoice_policy == 'delivery'
        )
        self.assertEqual(
            deliverables,
            sale_order_custom.order_line.linked_line_ids,
            "Only combo item lines should be invoiced on delivery.",
        )
        deliverables.qty_delivered = 3
        self.assertEqual(
            sale_order_custom.order_line.mapped('qty_to_invoice'),
            [3.0, 3.0, 3.0],
        ...
        self.assertRecordValues(sale_order_custom.order_line, [
            {
                'product_id': product_combo.id,
                'qty_to_invoice': 0,
                'qty_invoiced': 3,
    ...
    def test_qty_invoiced(self):
        ...
        # Let's do an invoice with invoiceable lines
        invoicing_wizard = self.env['sale_custom.advance.payment.inv'].with_context(self.context).create({
            'advance_payment_method': 'delivered'
        })
        ...
        quantity = 5.13
        move_form = Form(sale_order_custom.invoice_ids)
        with move_form.invoice_line_ids.edit(0) as line_form:
    ...
    def test_multi_company_invoice(self):
        """Checks that the company of the invoices generated in a multi company environment using the
           'sale_custom.advance.payment.inv' wizard fit with the company of the SO and not with the current company.
        """
        so_company_id = self.sale_order_custom.company_id.id
        yet_another_company_id = self.company_data_2['company'].id
        so_for_downpayment = self.sale_order_custom.copy()
        ...
        self.sale_order_custom.with_context(self.context).action_confirm()
        payment = self.env['sale_custom.advance.payment.inv'].with_context(no_journal_ctxt).create({
            'advance_payment_method': 'percentage',
            'amount': 50,
        })
        payment.create_invoices()
        self.assertEqual(self.sale_order_custom.invoice_ids[0].company_id.id, so_company_id, "The company of the invoice should be the same as the one from the SO")

        so_for_downpayment.with_context(context_for_downpayment).action_confirm()
        downpayment = self.env['sale_custom.advance.payment.inv'].with_context(context_for_downpayment).create({
            'advance_payment_method': 'fixed',
            'fixed_amount': 50,
        })
        downpayment.create_invoices()
    ...
    def test_invoice_analytic_distribution_model(self):
        ...
        so_form = Form(self.env['sale_custom.order'])
        ...
        down_payment = self.env['sale_custom.advance.payment.inv'].with_context(so_context).create({})
    ...
    def test_invoice_analytic_rule_with_account_prefix(self):
        ...
        so = self.env['sale_custom.order'].create({'partner_id': self.partner_a.id})
    ...
    def test_invoice_after_product_return_price_not_default(self):
        so = self.env['sale_custom.order'].create({
            'name': 'Sale order',
            'partner_id': self.partner_a.id,
            'partner_invoice_id': self.partner_a.id,
            'order_line': [
        ...
        invoicing_wizard = self.env['sale_custom.advance.payment.inv'].with_context(so_context).create({})
        invoicing_wizard.create_invoices()
    ...
    def test_group_invoice(self):
        ...
        so1 = self.sale_order_custom.with_context(mail_notrack=True).copy()
        ...
        wiz = self.env['sale_custom.advance.payment.inv'].with_context(active_ids=orders.ids, open_invoices=True).create({})
    ...
    def test_so_note_to_invoice(self):
        ...
        self.sale_order_custom.order_line = [Command.create({
            'name': 'This is a note',
            ...
            'product_uom': False,
            'price_unit': 0,
            'order_id': self.sale_order_custom.id,
            'tax_id': False,
        })]
        ...
        # create invoice
        invoice = self.sale_order_custom._create_invoices()
    ...
    def test_sale_order_standard_flow_with_invoicing(self):
        ...
        """
        self.sale_order_custom.order_line.product_uom_qty = 2.0
        # TODO?: validate invoice and register payments
        self.sale_order_custom.order_line.read(['name', 'price_unit', 'product_uom_qty', 'price_total'])

        self.assertEqual(self.sale_order_custom.amount_total, 1240.0, 'Sale: total amount is wrong')
        self.sale_order_custom.order_line._compute_product_updatable()
        self.assertTrue(self.sale_order_custom.order_line[0].product_updatable)
        # send quotation
        email_act = self.sale_order_custom.action_quotation_send()
        email_ctx = email_act.get('context', {})
        self.sale_order_custom.with_context(**email_ctx).message_post_with_source(
            self.env['mail.template'].browse(email_ctx.get('default_template_id')),
            subtype_xmlid='mail.mt_comment',
        )
        self.assertTrue(self.sale_order_custom.state == 'sent', 'Sale: state after sending is wrong')
        self.sale_order_custom.order_line._compute_product_updatable()
        self.assertTrue(self.sale_order_custom.order_line[0].product_updatable)

        # confirm quotation
        self.sale_order_custom.action_confirm()
        self.assertTrue(self.sale_order_custom.state == 'sale')
        self.assertTrue(self.sale_order_custom.invoice_status == 'to invoice')
        ...
        self.assertTrue(self.sale_order_custom.invoice_status == 'no', 'Sale: SO status after invoicing should be "nothing to invoice"')
        self.assertTrue(len(self.sale_order_custom.invoice_ids) == 1, 'Sale: invoice is missing')
        self.sale_order_custom.order_line._compute_product_updatable()
        self.assertFalse(self.sale_order_custom.order_line[0].product_updatable)
        ...
        self.assertTrue(self.sale_order_custom.invoice_status == 'to invoice', 'Sale: SO status after delivery should be "to invoice"')
        invoice2 = self.sale_order_custom._create_invoices()
        ...
        self.assertTrue(self.sale_order_custom.invoice_status == 'invoiced', 'Sale: SO status after invoicing everything should be "invoiced"')
        self.assertTrue(len(self.sale_order_custom.invoice_ids) == 2, 'Sale: invoice is missing')

        # go over the sold quantity
        self.sol_serv_order.write({'qty_delivered': 10})
        self.assertTrue(self.sale_order_custom.invoice_status == 'upselling', 'Sale: SO status after increasing delivered qty higher than ordered qty should be "upselling"')
        ...
        invoice3 = self.sale_order_custom._create_invoices()
        ...
        self.assertTrue(self.sale_order_custom.invoice_status == 'invoiced', 'Sale: SO status after invoicing everything (including the upsel) should be "invoiced"')
    ...
    def test_so_create_multicompany(self):
        ...
        so_1 = self.env['sale_custom.order'].with_user(self.company_data['default_user_salesman']).create({
    ...
    def test_partial_invoicing_interaction_with_invoicing_switch_threshold(self):
        ...
        sale_order = self.env['sale_custom.order'].create({
            'partner_id': self.partner_a.id,
            'order_line': [
                Command.create({
        ...
        line = sale_order_custom.order_line[0]

        sale_order_custom.action_confirm()

        line.qty_delivered = 10

        invoice = sale_order_custom._create_invoices()
        invoice.action_post()
        ...
        self.assertEqual(sale_order_custom.amount_to_invoice, 150)
    ...
    def test_salesperson_in_invoice_followers(self):
        ...
        sale_order_custom.action_confirm()
        invoice = sale_order_custom._create_invoices(final=True)
    ...
    def test_amount_to_invoice_multiple_so(self):
        ...
        """
        sale_order_1 = self.env['sale_custom.order'].create({
            'partner_id': self.partner_a.id,
            'order_line': [
                Command.create({
                    'product_id': self.company_data['product_delivery_no'].id,
        ...
        sale_order_2 = self.env['sale_custom.order'].create({
            'partner_id': self.partner_a.id,
            'order_line': [
                Command.create({
                    'product_id': self.company_data['product_delivery_no'].id,
        ...
        self.env['sale_custom.advance.payment.inv'].create({
            'advance_payment_method': 'delivered',
    ...
    def test_amount_to_invoice_price_unit_change(self):
        ...
        sol_prod_deliver = self.env['sale_custom.order.line'].create({
            'product_id': self.company_data['product_order_no'].id,
            'product_uom_qty': 5,
            'order_id': so.id,
            'tax_id': False,
        ...
        invoice_vals = self.env['sale_custom.advance.payment.inv'].create({
            'advance_payment_method': 'delivered',
    ...
    def test_invoice_line_name_has_product_name(self):
        ...
        so.action_confirm()
        inv = self.sale_order_custom._create_invoices()
    ...
    def test_credit_note_automatic_matching(self):
        ...
        sale_order_custom.order_line.qty_delivered = 1

        invoice = sale_order_custom._create_invoices()
        invoice.action_post()

        sale_order_custom.order_line.qty_delivered = 0
        ...
        with patch.object(self.env.registry['account.move'], '_refunds_origin_required', lambda move: True):
            ...
            credit_note = self.env['sale_custom.advance.payment.inv'].with_context(wizard_context).create({})._create_invoices(sale_order)
    ...
    def test_credit_note_no_automatic_matching(self):
        ...
        sale_line1 = self.env['sale_custom.order.line'].create({
            'product_id': product.id,
            'product_uom_qty': 1,
            'price_unit': 200.0,
            'order_id': sale_order_custom.id,
        })
        sale_line2 = self.env['sale_custom.order.line'].create({
            'product_id': product.id,
            'product_uom_qty': 1,
            'price_unit': 100.0,
            'order_id': sale_order_custom.id,
    ...
    def test_refund_salesteam(self):
        ...
        self.sale_order_custom.write({
            'user_id': salesperson,
            'team_id': team2.id,
        })

        # Set all prices to negative values to force a refund
        self.sale_order_custom.order_line.price_unit = -10
        self.sale_order_custom.action_confirm()
        invoice = self.sale_order_custom._create_invoices(final=True)
