...
class PaymentLinkWizard(models.TransientModel):
    ...
    def _compute_confirmation_message(self):
        ...
        for wizard in self.filtered(lambda w: w.res_model == 'sale_custom.order'):
            sale_order = wizard.env['sale_custom.order'].sudo().browse(wizard.res_id)
            if sale_order_custom.state in ('draft', 'sent') and sale_order_custom.require_payment:
                remaining_amount = sale_order_custom._get_prepayment_required_amount() - sale_order_custom.amount_paid
    ...
    def _prepare_query_params(self, *args):
        ...
        if self.res_model != 'sale_custom.order':
            return res
