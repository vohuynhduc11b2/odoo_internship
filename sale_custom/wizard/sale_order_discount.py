...
class SaleOrderDiscount(models.TransientModel):
    _name = 'sale_custom.order.discount'
    _description = "Discount Wizard"

    sale_order_id = fields.Many2one(
        'sale_custom.order', default=lambda self: self.env.context.get('active_id'), required=True)
    ...
    def _create_discount_lines(self):
        ...
        return self.env['sale_custom.order.line'].create(vals_list)
