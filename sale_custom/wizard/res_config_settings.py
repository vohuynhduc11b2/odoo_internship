...
class ResConfigSettings(models.TransientModel):
    ...
    group_auto_done_setting = fields.Boolean(
        string="Lock Confirmed Sales", implied_group='sale_custom.group_auto_done_setting')
    group_discount_per_so_line = fields.Boolean(
        string="Discounts", implied_group='sale_custom.group_discount_per_so_line')
    group_proforma_sales = fields.Boolean(
        string="Pro-Forma Invoice", implied_group='sale_custom.group_proforma_sales',
        help="Allows you to send pro-forma invoice.")
    group_warning_sale = fields.Boolean(
        string="Sale Order Warnings", implied_group='sale_custom.group_warning_sale')
    ...
    automatic_invoice = fields.Boolean(
        ...
        config_parameter='sale_custom.automatic_invoice',
    )
    ...
    invoice_mail_template_id = fields.Many2one(
        ...
        config_parameter='sale_custom.default_invoice_email_template',
    ...
    def set_values(self):
        ...
        if self.default_invoice_policy != 'order':
            self.env['ir.config_parameter'].set_param(key='sale_custom.automatic_invoice', value=False)
