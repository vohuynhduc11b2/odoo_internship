...
class WebsiteSale(payment_portal.PaymentPortal):
    ...
    def _get_additional_shop_values(self, values):
        """ Hook to update values used for rendering website_sale_custom.products template """
        return {}
    ...
    # ==========================================================
    # UC HELPERS: LOG SNAPSHOT + RE-APPLY PROMOS FOR CHECKOUT/PAYMENT
    # ==========================================================
    def _uc_dump_lines(self, order):
        """Dump lines để log theo dõi discount/promo trước-sau."""
        if not order:
            return []
        ...
        for l in lines:
            promo_ids = []
            if "promotion_ids" in l._fields:
                promo_ids = l.promotion_ids.ids
            out.append({
                "line_id": l.id,
                ...
                "linked_line_id": (l.linked_line_id.id if "linked_line_id" in l._fields and l.linked_line_id else False),
            })
        return out

    def _uc_apply_promos_for_checkout(self, order, where=""):
        """Re-apply promo pipeline để không bị _recompute_prices / update_pricelist reset discount."""
        if not order:
            _logger.info("[UC_%s] order=None => skip apply promos", where)
            return order

        order = order.sudo()
        _logger.info(
            "[UC_%s] BEFORE apply promos order=%s amount_total=%s lines=%s",
            where, order.id, order.amount_total, self._uc_dump_lines(order),
        )

        # 1) auto apply promotions (nếu có engine)
        try:
            if hasattr(order, "_auto_apply_promotions"):
                try:
                    order._auto_apply_promotions(for_website=True)
                except TypeError:
                    order._auto_apply_promotions()
        except Exception:
            _logger.exception("[UC_%s] _auto_apply_promotions failed order=%s", where, order.id)

        # 2) apply promotions to lines
        try:
            sale_lines = order.order_line.filtered(lambda l: l.product_id and not getattr(l, "display_type", False))
            if "is_gift" in sale_lines._fields:
                sale_lines = sale_lines.filtered(lambda l: not l.is_gift)
            # (giữ linked_line_id filter như bạn đang dùng cho engine, KHÔNG ảnh hưởng auto-purchase)
            if "linked_line_id" in sale_lines._fields:
                sale_lines = sale_lines.filtered(lambda l: not l.linked_line_id)

            if sale_lines and hasattr(sale_lines, "apply_promotions_to_line"):
                sale_lines.apply_promotions_to_line()
        except Exception:
            _logger.exception("[UC_%s] apply_promotions_to_line failed order=%s", where, order.id)

        # 3) auto purchase promo (chạy SAU CÙNG)
        try:
            if hasattr(order, "website_apply_auto_purchase_promo"):
                order.website_apply_auto_purchase_promo()
        except Exception:
            _logger.exception("[UC_%s] website_apply_auto_purchase_promo failed order=%s", where, order.id)
        ...
        try:
            ...
            _logger.exception("[UC_%s] compute amounts failed order=%s", where, order.id)

        _logger.info(
            "[UC_%s] AFTER apply promos order=%s amount_total=%s lines=%s",
            where, order.id, order.amount_total, self._uc_dump_lines(order),
        )
        return order

    def _uc_compute_amounts(self, order):
        try:
            if hasattr(order, "_compute_amounts"):
                order._compute_amounts()
            elif hasattr(order, "_amount_all"):
            ...
            _logger.exception("[UC_CHECKOUT] compute_amounts failed order=%s", order.id if order else None)


    def _uc_refresh_promos_before_payment(self, order_sudo, tag=""):
        """Re-apply KM sau khi _recompute_prices() (vì hàm này reset discount)."""
        if not order_sudo:
            return

        _logger.info("[UC_CHECKOUT] %s BEFORE refresh order=%s total=%s lines=%s",
                     tag, order_sudo.id, order_sudo.amount_total, self._uc_dump_lines(order_sudo))

        # 1) Auto apply promotions nếu module của bạn có
        try:
            if hasattr(order_sudo, "_auto_apply_promotions"):
                try:
                    order_sudo.sudo()._auto_apply_promotions(for_website=True)
                except TypeError:
                    order_sudo.sudo()._auto_apply_promotions()
        except Exception:
            _logger.exception("[UC_CHECKOUT] _auto_apply_promotions failed order=%s", order_sudo.id)

        # 2) Apply promotion engine lên line
        try:
            lines = order_sudo.order_line.filtered(lambda l: l.product_id and not getattr(l, "display_type", False))
            if lines and hasattr(lines, "apply_promotions_to_line"):
                lines.apply_promotions_to_line()
        except Exception:
            _logger.exception("[UC_CHECKOUT] apply_promotions_to_line failed order=%s", order_sudo.id)

        # 3) Gift/bundle helpers (nếu có)
        try:
            if hasattr(order_sudo, "update_applied_promotions"):
                order_sudo.update_applied_promotions()
            if hasattr(order_sudo, "resequence_order_lines"):
                order_sudo.resequence_order_lines()
        except Exception:
            _logger.exception("[UC_CHECKOUT] update_applied_promotions/resequence_order_lines failed order=%s", order_sudo.id)

        # 4) Auto-purchase (mua lần đầu/hai) — quan trọng nhất
        try:
            if hasattr(order_sudo, "website_apply_auto_purchase_promo"):
                order_sudo.website_apply_auto_purchase_promo()
        except Exception:
            _logger.exception("[UC_CHECKOUT] website_apply_auto_purchase_promo failed order=%s", order_sudo.id)

        # 5) Recompute totals
        self._uc_compute_amounts(order_sudo)

        _logger.info("[UC_CHECKOUT] %s AFTER  refresh order=%s total=%s lines=%s",
                     tag, order_sudo.id, order_sudo.amount_total, self._uc_dump_lines(order_sudo))
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, ppg=False, **post):
        ...
        filter_by_tags_enabled = website.is_view_active('website_sale_custom.filter_products_tags')
        ...
        # Build kwargs for QueryURL, remove search from post to avoid duplicate
        query_kwargs = self._shop_get_query_url_kwargs(category and int(category), search, min_price, max_price, **post)
        keep = QueryURL('/shop', **query_kwargs)
        ...
        filter_by_price_enabled = website.is_view_active('website_sale_custom.filter_products_price')
        ...
        if not layout_mode:
            if website.viewref('website_sale_custom.products_list_view').active:
                layout_mode = 'list'
            else:
                layout_mode = 'grid'
        ...
        return request.render("website_sale_custom.products", values)
    ...
    def product(self, product, category='', search='', **kwargs):
        ...
        return request.render("website_sale_custom.product", self._prepare_product_values(product, category, search, **kwargs))
    ...
    def _prepare_product_values(self, product, category, search, **kwargs):
        ...
        view_track = request.website.viewref("website_sale_custom.product").track
    ...
    def pricelist_change(self, pricelist, **post):
        ...
        if (
            ...
            if redirect_url and request.website.is_view_active('website_sale_custom.filter_products_price'):
    ...
    def _cart_values(self, **post):
        """
        This method is a hook to pass additional values when rendering the 'website_sale_custom.cart' template (e.g. add
    ...
    def cart(self, access_token=None, revive='', **post):
        ...
        values = {}
        if access_token:
            abandoned_order = request.env['sale_custom.order'].sudo().search([('access_token', '=', access_token)], limit=1)
        ...
        if order:
            try:
                ...
                _logger.exception("[UC_CART] sync website tier prices failed order=%s", order.id)
            order.order_line.filtered(lambda sol: sol.product_id and not sol.product_id.active).unlink()
            if not order.website_order_line or not (order.cart_quantity or 0):
                request.website.sale_reset()
                request.session['website_sale_cart_quantity'] = 0
                order = False
                values['website_sale_order'] = False
            else:
                values['suggested_products'] = order._cart_accessories()
                values.update(self._get_express_shop_payment_values(order))

        values.update(self._cart_values(**post))
        return request.render("website_sale_custom.cart", values)

    @http.route(['/shop/cart/update'], type='http', auth="public", methods=['POST'], website=True)
    def cart_update(
        self, product_id, line_id=None, add_qty=1, set_qty=None,
        ...
        if order and getattr(order, "state", "draft") != "draft":
            request.website.sale_reset()
            try:
                order = request.website.sale_get_order(force_create=True, update_pricelist=False)
            except TypeError:
        ...
        no_variant_attribute_values = (
            ...
            or kwargs.pop("no_variant_attribute_values", None)
        )
        if no_variant_attribute_values and no_variant_attribute_value_ids is None:
            data = json_scriptsafe.loads(no_variant_attribute_values)
            no_variant_attribute_value_ids = [int(x["value"]) for x in data]

        # QUAN TRỌNG: phân biệt form có gửi set_qty hay không
        raw_set_qty = request.httprequest.form.get("set_qty", None)
        raw_add_qty = request.httprequest.form.get("add_qty", None)

        add_qty_f = _to_float(raw_add_qty, _to_float(add_qty, 0.0) or 0.0) or 0.0
        set_qty_f = _to_float(raw_set_qty, None)  # None nếu không gửi; 0.0 nếu gửi '0' (xóa)

        _logger.info(
            "[UC_CART] /shop/cart/update order=%s product_id=%s line_id=%s add_qty=%s set_qty=%s ref=%s",
            getattr(order, "id", None), product_id, line_id, add_qty_f, set_qty_f, request.httprequest.referrer
        )

        # 1) Update cart (pass line_id để xóa đúng dòng)
        order._cart_update(
        ...
        # 2) ép chạy lại auto-purchase (đảm bảo line mới cũng ăn)
        try:
            order.website_apply_auto_purchase_promo()
        except Exception:
            _logger.exception("[UC_PROMO] website_apply_auto_purchase_promo failed in /shop/cart/update")
        ...
        redirect = kwargs.get("r") or request.httprequest.referrer or "/shop/cart"
        return request.redirect(redirect)
    ...
    def cart_update_json(self, product_id, line_id=None, add_qty=None, set_qty=None, display=True,
        ...
        def _default_payload(_order):
            return {
                "cart_quantity": (_order.cart_quantity if _order else 0) or 0,
                "notification_info": {
                    "message": "Đã thêm vào giỏ hàng",
                    "lines": [], "items": [], "products": [], "cart_lines": [], "order_lines": [],
                    "warning": "",
                    "currency_id": (_order.currency_id.id if _order else request.website.currency_id.id),
                },
            }
    
        _logger.info(
            "[UC_CART] /shop/cart/update_json order=%s product_id=%s line_id=%s add_qty=%s set_qty=%s",
            getattr(order, "id", None), product_id, line_id, add_qty, set_qty
        )
    
        try:
            if not order or getattr(order, "state", "draft") != "draft":
                request.website.sale_reset()
                return _default_payload(order)
            ...
            # ép chạy lại auto-purchase (đảm bảo line mới cũng ăn)
            try:
                order.website_apply_auto_purchase_promo()
            except Exception:
                _logger.exception("[UC_PROMO] website_apply_auto_purchase_promo failed in /shop/cart/update_json")
            ...
            updated_line_id = values.get("line_id")
            try:
                updated_line_id = int(updated_line_id) if updated_line_id else None
            except Exception:
                updated_line_id = None
    
            notif = self._get_cart_notification_information(order, [updated_line_id] if updated_line_id else []) or {}
            if not isinstance(notif, dict):
                notif = {}
    
            # normalize keys to avoid template crash
            for k in ("lines", "items", "products", "cart_lines", "order_lines"):
                if not isinstance(notif.get(k), list):
                    notif[k] = []
    
            notif["warning"] = values.pop("warning", "") or ""
            notif.setdefault("currency_id", order.currency_id.id)
            notif.setdefault("message", "Đã thêm vào giỏ hàng")
    
            # ===== Fallback: nếu notif.lines rỗng thì tự build từ line vừa update =====
            try:
                if (not notif.get("lines")) and updated_line_id:
                    line = request.env["sale.order.line"].sudo().browse(updated_line_id).exists()
                    if line:
                        prod = line.product_id
                        qty = float(line.product_uom_qty or 0.0)
    
                        # message có tên sp
                        notif["message"] = f"Đã thêm {prod.display_name} (x{qty:g}) vào giỏ hàng"
    
                        notif["lines"] = [{
                            "id": line.id,
                            "linked_line_id": getattr(line, "linked_line_id", False) and line.linked_line_id.id or False,
                            "image_url": f"/web/image/product.product/{prod.id}/image_128",
                            "quantity": qty,
                            "name": prod.display_name,
                            "description": (line.name and line.name != prod.display_name) and line.name or "",
                            "line_price_total": float(line.price_total or 0.0),
                        }]
                    else:
                        # vẫn có message theo product_id
                        prod = request.env["product.product"].sudo().browse(int(product_id)).exists()
                        if prod:
                            notif["message"] = f"Đã thêm {prod.display_name} vào giỏ hàng"
            except Exception:
                _logger.exception("[UC_CART] build notification fallback failed")
                notif.setdefault("message", "Đã thêm vào giỏ hàng")
    
            values["cart_quantity"] = order.cart_quantity or 0
            values["notification_info"] = notif
            request.session["website_sale_cart_quantity"] = order.cart_quantity
    
            if hasattr(order, "website_auto_purchase_stage"):
                values["auto_purchase_stage"] = order.website_auto_purchase_stage or ""
            if hasattr(order, "website_auto_purchase_promo_id"):
                values["auto_purchase_promo_id"] = order.website_auto_purchase_promo_id.id if order.website_auto_purchase_promo_id else 0
    
            if not order.website_order_line or not (order.cart_quantity or 0):
                request.website.sale_reset()
                return {**_default_payload(order), **values}
            ...
            values["website_sale_custom.cart_lines"] = request.env["ir.ui.view"]._render_template(
                "website_sale_custom.cart_lines",
                {"website_sale_order": order, "date": fields.Date.today(), "suggested_products": order._cart_accessories()}
            )
            values["website_sale_custom.total"] = request.env["ir.ui.view"]._render_template(
                "website_sale_custom.total", {"website_sale_order": order}
            )
            ...
            _logger.exception("cart_update_json failed: %s", e)
            return _default_payload(order)
    ...
    def _oms_reset_cart_session(self, *, keep_transaction=False, keep_last_order=False, last_order_id=None):
        """Reset website cart-related session keys.

        - keep_transaction: giữ `sale_transaction_id` nếu vẫn cần quay về từ cổng thanh toán
        - keep_last_order: giữ `sale_last_order_id` để còn xem /shop/confirmation
        """
        # Clear current cart
        ...
        if not keep_transaction:
            request.session.pop('sale_transaction_id', None)

        if last_order_id is not None:
            request.session['sale_last_order_id'] = int(last_order_id)
        elif not keep_last_order:
            request.session.pop('sale_last_order_id', None)

        # Custom session messages (optional cleanup)
        request.session.pop('uc_voucher_msg', None)
        request.session.pop('uc_cart_msg', None)

    def _oms_clear_order_lines(self, order_sudo):
        """Clear all order lines in sudo mode, including delivery/promotions/gifts."""
        if not order_sudo:
            return

        # Remove voucher/coupon if your custom module stores it on the order
        if hasattr(order_sudo, 'website_clear_voucher'):
            try:
                order_sudo.website_clear_voucher()
            except Exception:
                # Don't block cart clearing on voucher issues
                pass

        # Unlink all lines in one go (your custom unlink should cascade gifts/bundles)
        order_sudo.order_line.unlink()

        # Reset delivery method if any
        if 'carrier_id' in order_sudo._fields:
            order_sudo.carrier_id = False

    @route(['/shop/cart/clear'], type='json', auth="public", website=True)
    def clear_cart(self):
        """Clear the current website cart (lines + session)."""
        order = request.website.sale_get_order(force_create=False)
        if order:
            self._oms_clear_order_lines(order.sudo())

        # Fully reset session for cart
        self._oms_reset_cart_session(keep_transaction=False, keep_last_order=False)
    ...
    @route(['/shop/cart/clear_http'], type='http', auth="public", website=True, methods=['POST'], csrf=True)
    def clear_cart_http(self, **post):
        """HTTP endpoint for the 'Clear cart' button (redirect back to cart)."""
        self.clear_cart()
        request.session['uc_cart_msg'] = _('Đã xóa giỏ hàng.')
        return request.redirect('/shop/cart')


    @route(['/shop/pay_now'], type='http', auth='public', website=True, methods=['POST'], csrf=True, sitemap=False)
    def shop_pay_now(self, **post):
        """Cart 'Pay now': go through checkout quickly and skip the extra_info step."""
        order_sudo = request.website.sale_get_order()
        if not order_sudo or order_sudo.state != 'draft' or not order_sudo.order_line:
            return request.redirect('/shop/cart')

        # Flag used in /shop/confirm_order to bypass the extra step when enabled.
        request.session['uc_skip_extra_info'] = True

        # When Pay now is used, we want to clear the cart session as soon as the payment
        # transaction is created (so the customer can continue shopping in another tab).
        request.session['uc_clear_cart_on_paynow'] = True
        request.session['sale_last_order_id'] = order_sudo.id

        return request.redirect('/shop/checkout?try_skip_step=true')


    @route('/shop/payment/transaction/<int:sale_order_id>', type='json', auth='public', website=True, sitemap=False)
    def shop_payment_transaction(self, sale_order_id, access_token=None, **kwargs):
        """Wrap the standard transaction creation to auto-clear cart & (optionally) validate order.

        Goal (requested): when the customer clicks **Pay now**, the current cart session should be
        cleared automatically and the order should be created/kept as a real order record.

        We only auto-clear when the session flag `uc_clear_cart_on_paynow` is set (set by /shop/pay_now).
        """
        parent = super()
        order_sudo = request.env['sale_custom.order'].sudo().browse(int(sale_order_id)).exists()
        if order_sudo and hasattr(order_sudo, "_uc_wait_sales_price_lines"):
            wait_lines = order_sudo._uc_wait_sales_price_lines()
            if wait_lines:
        ...
        # Delegate to the original implementation (method name/signature can differ across versions).
        if hasattr(parent, 'shop_payment_transaction'):
            try:
                res = parent.shop_payment_transaction(sale_order_id, access_token=access_token, **kwargs)
            except TypeError:
                res = parent.shop_payment_transaction(sale_order_id, **kwargs)
        elif hasattr(parent, 'payment_transaction'):
            try:
                res = parent.payment_transaction(sale_order_id, access_token=access_token, **kwargs)
            except TypeError:
                res = parent.payment_transaction(sale_order_id, **kwargs)
        else:
            raise NotFound()

        # Auto-clear cart right after transaction creation (requested behavior)
        if True:
            # Ensure we can still display /shop/confirmation later even if `sale_order_id` is cleared.
            request.session['sale_last_order_id'] = int(sale_order_id)

            # OPTIONAL: confirm the order right away (so "tạo đơn" happens even for offline methods).
            # If you don't want early confirmation, comment this block.
            try:
                if order_sudo and order_sudo.state in ('draft', 'sent'):
                    order_sudo._check_cart_is_ready_to_be_paid()
                    order_sudo._validate_order()
            except Exception:
                _logger.exception("PayNow: failed to validate order %s", sale_order_id)

            # Clear cart session keys so the customer gets a fresh cart immediately.
            self._oms_reset_cart_session(
                keep_transaction=False,
                keep_last_order=True,
                last_order_id=sale_order_id,
            )

            # Pre-create a new empty cart for continued shopping (optional but convenient).
            try:
    ...
    def _get_cart_notification_information(self, order, line_ids):
        """Support sale_custom.order.line ids."""
        line_ids = [lid for lid in (line_ids or []) if lid]
        lines = request.env['sale_custom.order.line'].sudo().browse(line_ids).exists()
        ...
        for l in lines:
            notif_lines.append({
                ...
                "price": l.price_unit,  # hoặc l.price_subtotal tùy anh muốn hiển thị
                "image_url": "/web/image/product.product/%s/image_128" % (l.product_id.id),
            })

        # QUAN TRỌNG: CartNotification.template đang đọc props.lines.length
        return {
            "lines": notif_lines,
            # thêm các key khác nếu front của anh dùng
    ...
    def shop_confirm_order(self, **post):
        # IMPORTANT: tránh sale_get_order tự update_pricelist làm reset discount
        try:
            order_sudo = request.website.sale_get_order(update_pricelist=False)
        except TypeError:
            order_sudo = request.website.sale_get_order()

        _logger.info("[UC_CONFIRM] enter confirm_order order=%s", order_sudo.id if order_sudo else None)

        if redirection := self._check_cart_and_addresses(order_sudo):
            _logger.info("[UC_CONFIRM] redirect by _check_cart_and_addresses order=%s", order_sudo.id if order_sudo else None)
            return redirection

        # Core recompute (giữ lại nếu bạn cần taxes/price_unit đúng theo fiscal/shipping)
        try:
            order_sudo._recompute_taxes()
        except Exception:
            _logger.exception("[UC_CONFIRM] _recompute_taxes failed order=%s", order_sudo.id)

        # Preserve line prices entered by Sales. Recomputing prices here would
        # apply the website pricelist again and overwrite quoted contact-price lines.

        # ✅ FIX: apply lại promo/auto-purchase SAU recompute_prices để discount không bị reset khi qua Payment
        self._uc_apply_promos_for_checkout(order_sudo, where="CONFIRM_ORDER")

        extra_step = request.website.viewref('website_sale_custom.extra_info')
        skip_extra = bool(request.session.pop('uc_skip_extra_info', False))
        if extra_step.active and not skip_extra:
            _logger.info("[UC_CONFIRM] redirect extra_info order=%s", order_sudo.id)
            return request.redirect("/shop/extra_info")

        _logger.info("[UC_CONFIRM] redirect payment order=%s", order_sudo.id)
    ...
    def shop_checkout(self, try_skip_step=None, **query_params):
        ...
        return request.render('website_sale_custom.checkout', checkout_page_values)
    ...
    def _prepare_checkout_page_values(self, order_sudo, **_kwargs):
        ...
        :param sale_custom.order order_sudo: The current cart.
        ...
        return {
            ...
            'json_pickup_location_data': json.dumps(getattr(order_sudo, "pickup_location_data", None) or {}),
        }
    ...
    def shop_address(
        ...
        return request.render('website_sale_custom.address', address_form_values)
    ...
    def _create_new_address(
        ...
        :param order_sudo: the current cart, as a sudoed `sale_custom.order` recordset
    ...
    def _save_oms_checkout_notes(self, order_sudo, note):
        note = (note or '').strip()
        if not order_sudo or not note:
            return

        vals = {}
        for field_name in ('dispatch_note', 'Comments', 'NoteInternal'):
            if field_name in order_sudo._fields:
                vals[field_name] = note
        if vals:
            order_sudo.sudo().write(vals)

    @route('/shop/save_checkout_notes', type='json', auth='public', website=True)
    def shop_save_checkout_notes(self, note=None, **kw):
        order_sudo = request.website.sale_get_order()
        if not order_sudo:
            return {'ok': False}

        self._save_oms_checkout_notes(order_sudo, note)
        return {'ok': True}
    ...
    def shop_payment(self, **post):
        ...
        _logger.info("[UC_PAYMENT] enter shop_payment order=%s", order_sudo.id if order_sudo else None)

        if redirection := self._check_cart_and_addresses(order_sudo):
            _logger.info("[UC_PAYMENT] redirect by _check_cart_and_addresses order=%s", order_sudo.id if order_sudo else None)
            return redirection

        if redirection := self._check_shipping_method(order_sudo):
            _logger.info("[UC_PAYMENT] redirect by _check_shipping_method order=%s", order_sudo.id if order_sudo else None)
            return redirection

        # ✅ FIX: đảm bảo discount/promo/auto-purchase đã “chốt” trước khi render payment page
        self._uc_apply_promos_for_checkout(order_sudo, where="PAYMENT_PAGE")
        ...
        _logger.info(
            "[UC_PAYMENT] render payment order=%s amount_total=%s",
            order_sudo.id if order_sudo else None,
            order_sudo.amount_total if order_sudo else None
        )
        return request.render("website_sale_custom.payment", render_values)

    @route(['/shop/extra_info'], type='http', auth="public", website=True, sitemap=False)
    def extra_info(self, **post):
        # If user pressed 'Pay now' in cart, bypass the extra step.
        if request.session.pop('uc_skip_extra_info', False):
            return request.redirect('/shop/payment')
        ...
        return request.render("website_sale_custom.extra_info", values)
    ...
    def _get_express_shop_payment_values(self, order, **kwargs):
        ...
        amount_wo_delivery = order._get_amount_total_excluding_delivery()
        delivery_amount = order.amount_total - amount_wo_delivery

        payment_form_values.update({
            'payment_access_token': payment_form_values.pop('access_token'),
            'minor_amount': payment_utils.to_minor_currency_units(amount_wo_delivery, order.currency_id),
            ...
            # compatibility field (todo remove in master)
            'delivery_amount': payment_utils.to_minor_currency_units(delivery_amount, order.currency_id),
    ...
    def _get_shop_payment_errors(self, order):
        ...
        :param sale_custom.order order: The sales order to pay
        ...
        if order and hasattr(order, "_uc_wait_sales_price_lines"):
            wait_lines = order._uc_wait_sales_price_lines()
            if wait_lines:
                product_names = ", ".join(wait_lines.mapped("product_id.display_name")[:5])
                errors.append((
                    _("Chờ Sales nhập giá"),
                    _("Một số sản phẩm trong giỏ hàng đang để giá liên hệ. Sales sẽ nhập giá cho: %s. Sau khi có giá, bạn có thể tiếp tục thanh toán.") % product_names,
                ))
        return errors
    ...
    def shop_payment(self, **post):
        ...
        # ==========================================================
        # UC FIX: qua payment bị "mất giảm" -> re-apply KM trước khi render
        # ==========================================================
        try:
            _logger.info("[UC_CHECKOUT] shop_payment ENTER order=%s total=%s", order_sudo.id if order_sudo else None, getattr(order_sudo, "amount_total", None))

            # 1) Auto-purchase (mua lần đầu/hai)
            if order_sudo and hasattr(order_sudo, "website_apply_auto_purchase_promo"):
                order_sudo.website_apply_auto_purchase_promo()

            # 2) Apply promotion engine lên line (voucher/manual/auto)
            if order_sudo:
                lines = order_sudo.order_line.filtered(lambda l: l.product_id and not getattr(l, "display_type", False))
                if lines and hasattr(lines, "apply_promotions_to_line"):
                    lines.apply_promotions_to_line()

            # 3) Helper gift/bundle nếu có
            if order_sudo:
                if hasattr(order_sudo, "update_applied_promotions"):
                    order_sudo.update_applied_promotions()
                if hasattr(order_sudo, "resequence_order_lines"):
            ...
            # 4) Recompute totals
            if order_sudo:
                if hasattr(order_sudo, "_compute_amounts"):
                    order_sudo._compute_amounts()
                elif hasattr(order_sudo, "_amount_all"):
                    order_sudo._amount_all()

            _logger.info("[UC_CHECKOUT] shop_payment AFTER reapply order=%s total=%s",
                         order_sudo.id if order_sudo else None,
                         getattr(order_sudo, "amount_total", None))
        except Exception:
            _logger.exception("[UC_CHECKOUT] shop_payment reapply failed")
    ...
    def shop_payment_validate(self, sale_order_id=None, **post):
        ...
        if sale_order_id is None:
            ...
            if not order and 'sale_last_order_id' in request.session:
                ...
                order = request.env['sale_custom.order'].sudo().browse(last_order_id).exists()
        else:
            order = request.env['sale_custom.order'].sudo().browse(sale_order_id)
    ...
    def shop_payment_confirmation(self, **post):
        ...
        the status of a sale_custom.order. State at this point :

         - should not have any context / session info: clean them
         - take a sale_custom.order id, because we request a sale_custom.order and are not
        ...
        if sale_order_id:
            ...
            return request.render("website_sale_custom.confirmation", values)
        return request.redirect('/shop')
    ...
    def _check_cart_and_addresses(self, order_sudo):
        ...
        :param sale_custom.order order_sudo: The cart to check.
    ...
    # ==========================================================
    # UC helpers: log snapshot + refresh promo before checkout/payment
    # ==========================================================
    def _uc_lines_snapshot(self, order):
        """Snapshot gọn để log."""
        try:
            if not order:
                return []
            ...
            _logger.exception("[UC_CHECKOUT] snapshot failed order=%s", getattr(order, "id", None))
            return []

    def _uc_refresh_before_payment(self, order, where=""):
        """
        Refresh lại taxes/prices rồi ensure auto-purchase promo được áp lại (tránh bị _recompute_prices reset).
        Chỉ log + chạy safe, không ném lỗi ra UI.
        """
        if not order:
            return
        ...
        try:
            stage = getattr(order, "website_auto_purchase_stage", False)
            promo = getattr(order, "website_auto_purchase_promo_id", False)
            _logger.info(
                "[UC_CHECKOUT] %s BEFORE order=%s stage=%s promo=%s amount_total=%s lines=%s",
                where, order.id, stage, promo.id if promo else False, order.amount_total, self._uc_lines_snapshot(order)
            )
        except Exception:
            _logger.exception("[UC_CHECKOUT] %s BEFORE log failed order=%s", where, getattr(order, "id", None))
        ...
        try:
            ...
            _logger.exception("[UC_CHECKOUT] %s _recompute_taxes failed order=%s", where, order.id)

        # 2) recompute prices (quan trọng: dùng context để hạn chế compute discount/pricelist overwrite nếu bạn có check key này)
        # Preserve line prices entered by Sales. Do not recompute price_unit
        # during payment refresh.

        # 3) APPLY lại auto purchase promo sau recompute (đây là điểm fix “qua payment bị mất giảm”)
        try:
            if hasattr(order, "website_apply_auto_purchase_promo"):
                order.website_apply_auto_purchase_promo()
        except Exception:
            _logger.exception("[UC_CHECKOUT] %s website_apply_auto_purchase_promo failed order=%s", where, order.id)
        ...
        try:
            ...
            _logger.exception("[UC_CHECKOUT] %s compute amounts failed order=%s", where, order.id)

        try:
            stage = getattr(order, "website_auto_purchase_stage", False)
            promo = getattr(order, "website_auto_purchase_promo_id", False)
            _logger.info(
                "[UC_CHECKOUT] %s AFTER  order=%s stage=%s promo=%s amount_total=%s lines=%s",
                where, order.id, stage, promo.id if promo else False, order.amount_total, self._uc_lines_snapshot(order)
            )
        except Exception:
            _logger.exception("[UC_CHECKOUT] %s AFTER log failed order=%s", where, getattr(order, "id", None))
    ...
    def _check_addresses(self, order_sudo):
        ...
        :param sale_custom.order order_sudo: The cart whose addresses to check.
