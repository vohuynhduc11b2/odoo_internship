...
class ResConfigSettings(models.TransientModel):
    ...
    group_show_uom_price = fields.Boolean(
        ...
        default=False,
        implied_group="website_sale_custom.group_show_uom_price",
        group='base.group_portal,base.group_user,base.group_public',
    )
    group_product_price_comparison = fields.Boolean(
        string="Comparison Price",
        implied_group="website_sale_custom.group_product_price_comparison",
        group='base.group_portal,base.group_user,base.group_public',
    ...
    def _compute_checkout_process_steps(self):
        ...
        for record in self:
            ...
            record.enabled_extra_checkout_step = website.is_view_active(
                'website_sale_custom.extra_info'
            )
            record.enabled_buy_now_button = website.is_view_active(
                'website_sale_custom.product_buy_now'
            )
    ...
    def set_values(self):
        ...
        if self.website_id:
            ...
            extra_step_view = website.viewref('website_sale_custom.extra_info')
            buy_now_view = website.viewref('website_sale_custom.product_buy_now')
    ...
    def action_open_abandoned_cart_mail_template(self):
        return {
            ...
            'view_id': False,
            'view_mode': 'form',
            'res_id': self.env['ir.model.data']._xmlid_to_res_id("website_sale_custom.mail_template_sale_cart_recovery"),
        }
