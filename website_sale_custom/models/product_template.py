...
class ProductTemplate(models.Model):
    ...
    def _get_sales_prices(self, website):
        ...
        comparison_prices_enabled = self.env.user.has_group('website_sale_custom.group_product_price_comparison')

        res = {}
        for template in self:
            pricelist_price, pricelist_rule_id = pricelist_prices[template.id]
            oms_price_applied = False
            product = template.product_variant_id
            if product and hasattr(product, "uc_get_tier_price_unit"):
                oms_price = product.uc_get_tier_price_unit(
                    qty=1.0,
                    pricelist=pricelist,
                )
                if oms_price is None and product._is_oms_managed_pricelist(pricelist):
                    oms_price = 0.0
                if oms_price is not None:
                    pricelist_price = float(oms_price)
                    oms_price_applied = True
            ...
            base_price = None
            template_price_vals = {
                'price_reduce': pricelist_price if oms_price_applied else self._apply_taxes_to_price(
                    pricelist_price,
                    currency,
                    product_taxes,
                    taxes,
            ...
            if pricelist_item._show_discount_on_shop():
                ...
                if not oms_price_applied and currency.compare_amounts(pricelist_base_price, pricelist_price) == 1:
    ...
    def _get_additionnal_combination_info(self, product_or_template, quantity, date, website):
        ...
        oms_product = product_or_template
        if oms_product._name == "product.template":
            oms_product = oms_product.product_variant_id

        oms_price_applied = False
        if oms_product and hasattr(oms_product, "uc_get_tier_price_unit"):
            oms_price = oms_product.uc_get_tier_price_unit(
                qty=quantity or 1.0,
                pricelist=pricelist,
            )
            if oms_price is None and oms_product._is_oms_managed_pricelist(pricelist):
                oms_price = 0.0
            if oms_price is not None:
                pricelist_price = float(oms_price)
                price_before_discount = pricelist_price
        ...
        if (
            ...
            and self.env.user.has_group('website_sale_custom.group_product_price_comparison')
        ):
        ...
        if product_taxes and not oms_price_applied:
            taxes = fiscal_position.map_tax(product_taxes)
        ...
        uc_wait_sales_price = bool(
            oms_price_applied
            and (
                float_is_zero(combination_info['price'], precision_rounding=currency.rounding)
                or float(combination_info['price'] or 0.0) <= 1.0
            )
        )
        ...
        combination_info.update({
            'prevent_zero_price_sale': (
                ...
                and not uc_wait_sales_price
                and float_is_zero(
                    combination_info['price'],
                    precision_rounding=currency.rounding,
                )
            ),
            'uc_wait_sales_price': uc_wait_sales_price,
        ...
        if order:
            ...
            if line:
                qty_in_order = line.product_uom_qty

        combination_info["quantity"] = qty_in_order
    ...
    def _get_alternative_product_filter(self):
        return self.env.ref('website_sale_custom.dynamic_filter_cross_selling_alternative_products').id
        ...
        for product, data in zip(self, results_data):
            ...
            if with_category and categ_ids:
                data['category'] = self.env['ir.ui.view'].sudo()._render_template(
                    "website_sale_custom.product_category_extra_link",
                    {'categories': categ_ids, 'slug': self.env['ir.http']._slug}
                )
        return results_data

    def _search_render_results_prices(self, mapping, combination_info):
        if combination_info.get('prevent_zero_price_sale') or combination_info.get('uc_wait_sales_price'):
            return _("Giá liên hệ"), None
    ...
    def _website_show_quick_add(self):
        ...
        if not self.sale_ok:
            return False
        combination_info = self._get_combination_info(only_template=True)
    ...
    def _get_configurator_display_price(
        ...
        if oms_product and hasattr(oms_product, "uc_get_tier_price_unit"):
            ...
            if oms_price is None and oms_product._is_oms_managed_pricelist(pricelist):
                oms_price = 0.0
            if oms_price is not None:
                price = float(oms_price)
                pricelist_rule_id = False
                oms_price_applied = True

        if oms_price_applied:
