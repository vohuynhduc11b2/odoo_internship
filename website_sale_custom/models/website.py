...
class Website(models.Model):
    ...
    def _default_recovery_mail_template(self):
        try:
            return self.env.ref('website_sale_custom.mail_template_sale_cart_recovery').id
        except ValueError:
            return False
    ...
    cart_recovery_mail_template_id = fields.Many2one(
        ...
        domain=[('model', '=', 'sale_custom.order')],
    ...
    shop_ppg = fields.Integer(
        string="Number of products in the grid on the shop", default=48,
    )
    ...
    def _get_current_pricelist(self):
        ...
        try:
            ...
            pricing_partner = (
                partner_sudo._oms_get_strategic_pricing_partner()
                if hasattr(partner_sudo, "_oms_get_strategic_pricing_partner")
                else self.env['res.partner']
            )
            if pricing_partner:
                mode = (
                    request.session.get("oms_payment_mode")
                    or request.session.get("website_payment_type")
                ) if request else ""
                mode = (mode or "").strip().lower()
                if mode:
                    prepaid_modes = ("full", "deposit", "unc", "qr", "prepaid", "prepay")
                    strategic_pricelist = (
                        pricing_partner.oms_prepaid_pricelist_id
                        if mode in prepaid_modes
                        else pricing_partner.oms_debt_pricelist_id
                    )
                    strategic_pricelist = strategic_pricelist or pricing_partner.oms_debt_pricelist_id
                else:
                    if pricing_partner.oms_prepaid_pricelist_id and not pricing_partner.oms_debt_pricelist_id:
                        strategic_pricelist = pricing_partner.oms_prepaid_pricelist_id
                    elif pricing_partner.oms_debt_pricelist_id and not pricing_partner.oms_prepaid_pricelist_id:
                        strategic_pricelist = pricing_partner.oms_debt_pricelist_id
                    else:
                        strategic_pricelist = False
                if (
                    strategic_pricelist
                    and strategic_pricelist._is_available_on_website(self)
                    and strategic_pricelist._is_available_in_country(self._get_geoip_country_code())
                ):
                    pricelist = strategic_pricelist
        except Exception:
            pass
    ...
    def sale_product_domain(self):
        """Compatibility wrapper used by website_sale_custom custom search code."""
        return self._product_domain()
    ...
    def sale_get_order(self, force_create=False):
        ...
        :returns: current cart, as a sudoed `sale_custom.order` recordset (might be empty)
        """
        self.ensure_one()
        self = self.with_company(self.company_id)
    
        SaleOrder = self.env['sale_custom.order'].sudo()
    
        def _uc_reset_cart_session():
            """Clear session cart dứt điểm để không hồi lại đơn cũ."""
            if not request:
                return
        ...
        def _uc_is_order_not_cart_anymore(so):
            """Đơn không còn là cart editable -> phải detach."""
            if not so:
                return False
    
            # 1) các cờ custom của bạn
            if getattr(so, 'uc_website_finalized', False):
                return True
            if getattr(so, 'locked', False):
                return True
    
            # 2) state/approval_state (nếu có)
            if 'state' in so._fields and (so.state or 'draft') != 'draft':
                return True
    
            approval_state = getattr(so, 'approval_state', False)
            if approval_state and approval_state != 'draft':
                return True
    
            # 3) payment transaction: coi như payment initiated => không cho dùng lại cart
            try:
                tx = so.get_portal_last_transaction()
                tx_state = tx.state if tx else False
            except Exception:
                tx_state = False
    
            if tx_state in ('draft', 'pending', 'authorized', 'done'):
                return True
        ...
        # 1) ưu tiên lấy theo session
        if sale_order_id:
            sale_order_sudo = SaleOrder.browse(sale_order_id).exists()
        # 2) fallback: last_website_so_id (logged user)
        ...
        # ==========================================================
        # ✅ FIX CHÍNH: DETACH CART CŨ nếu đã chốt/locked/đang payment
        # ==========================================================
        if sale_order_sudo and _uc_is_order_not_cart_anymore(sale_order_sudo):
            _uc_reset_cart_session()
            sale_order_sudo = SaleOrder  # empty recordset
    
        # Không có cart và không yêu cầu tạo mới
        ...
        # Existing cart: sync session nếu thiếu
        if request and not request.session.get('sale_order_id'):
            request.session['sale_order_id'] = sale_order_sudo.id
            request.session['website_sale_cart_quantity'] = sale_order_sudo.cart_quantity
    
        # check change partner (after signup)
    ...
    def _send_abandoned_cart_email(self):
        for website in self.search([]):
            if not website.send_abandoned_cart_email:
                continue
            all_abandoned_carts = self.env['sale_custom.order'].search([
            ...
            for sale_order in abandoned_carts:
                template = self.env.ref('website_sale_custom.mail_template_sale_cart_recovery')
                ...
                email_vals = {} if template.email_to or template.partner_to else {
                    'email_to': sale_order_custom.partner_id.email_formatted
                }
                template.send_mail(sale_order_custom.id, email_values=email_vals)
                sale_order_custom.cart_recovery_email_sent = True
