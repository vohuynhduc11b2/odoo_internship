...
from odoo.addons.website_sale_custom.tests.common import WebsiteSaleCommon
...
class TestWebsitePriceList(WebsiteSaleCommon):
    ...
    def setUp(self):
        super().setUp()
        patcher = patch('odoo.addons.website_sale_custom.models.website.Website.get_pricelist_available', wraps=self._get_pricelist_available)
    ...
    def test_website_manual_price_preserved_when_applying_promotions(self):
        sale_order = self._create_so()
        line = sale_order.order_line[:1]
        original_price = float(line.price_unit)
        manual_price = original_price + 100.0
        line.write({'price_unit': manual_price})

        self.assertNotEqual(line.technical_price_unit, line.price_unit)
        self.assertTrue(line.with_context(website_id=self.website.id)._is_manual_price())

        promo = self.env['oms.promotion'].create({
            'name': 'Test Manual Price Promo',
            'discount_type': 'percent',
            'discount_percent': 20.0,
            'valid_from': fields.Date.today() - timedelta(days=1),
            'valid_to': fields.Date.today() + timedelta(days=1),
        })
        line = line.with_context(website_id=self.website.id)
        line.promotion_ids = [(4, promo.id)]

        line.apply_promotions_to_line()
        self.assertEqual(line.price_unit, manual_price)
    ...
    def test_pricelist_anonymous_user(self):
        ...
        order_sudo = self.env['sale_custom.order'].sudo().create({
...
class TestWebsitePriceListAvailableGeoIP(TestWebsitePriceListAvailable):
    ...
    def test_get_pricelist_available_geoip(self):
        ...
        with patch('odoo.addons.website_sale_custom.models.website.Website._get_geoip_country_code', return_value=self.BE.code):
            pls = self.website.get_pricelist_available()
    ...
    def test_get_pricelist_available_geoip4(self):
        ...
        with patch('odoo.addons.website_sale_custom.models.website.Website._get_geoip_country_code', return_value=self.BE.code), \
            patch('odoo.addons.website_sale_custom.models.website.Website._get_cached_pricelist_id', return_value=current_pl.id):
            pls = self.website.get_pricelist_available(show_visible=True)
    ...
    def test_get_pricelist_available_geoip5(self):
        ...
        with patch('odoo.addons.website_sale_custom.models.website.Website._get_geoip_country_code', return_value=self.US.code):
    ...
    def test_get_pricelist_available_geoip6(self):
        ...
        with patch(
            'odoo.addons.website_sale_custom.models.website.Website._get_geoip_country_code',
...
class TestWebsiteSaleSession(HttpCaseWithUserPortal):
    ...
    def test_update_pricelist_user_session(self):
        ...
        self.start_tour("/shop", 'website_sale_custom.website_sale_shop_pricelist_tour', login="")
