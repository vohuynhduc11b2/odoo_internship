...
class TestWebsiteSaleImage(HttpCase):
    ...
    def test_01_admin_shop_zoom_tour(self):
        ...
        self.env['ir.ui.view'].with_context(active_test=False).search(
            [('key', 'in', ('website_sale_custom.product_picture_magnify_hover', 'website_sale_custom.product_picture_magnify_click', 'website_sale_custom.product_picture_magnify_both'))]
        ).write({'active': False})
        self.env['ir.ui.view'].with_context(active_test=False).search(
            [('key', '=', 'website_sale_custom.product_picture_magnify_click')]
        ).write({'active': True})
