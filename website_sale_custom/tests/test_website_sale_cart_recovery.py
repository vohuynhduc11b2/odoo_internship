...
class TestWebsiteSaleCartRecoveryServer(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.recovery_template_default = cls.env.ref('website_sale_custom.mail_template_sale_cart_recovery')
        ...
        cls.so0 = cls.env['sale_custom.order'].create({
            'partner_id': cls.customer.id,
            'website_id': cls.website0.id,
            'is_abandoned_cart': True,
            'cart_recovery_email_sent': False,
        })
        cls.so1 = cls.env['sale_custom.order'].create({
            'partner_id': cls.customer.id,
            'website_id': cls.website1.id,
            'is_abandoned_cart': True,
            'cart_recovery_email_sent': False,
        })
        cls.so2 = cls.env['sale_custom.order'].create({
            'partner_id': cls.customer.id,
