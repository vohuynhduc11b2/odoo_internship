...
class TestWebsiteSaleCartAbandoned(TestWebsiteSaleCartAbandonedCommon):
    ...
    def setUpClass(cls):
        ...
        cls.so0before = cls.env['sale_custom.order'].create({
        ...
        cls.so0after = cls.env['sale_custom.order'].create({
            'partner_id': cls.customer.id,
            'website_id': cls.website0.id,
            'state': 'draft',
            'date_order': (now - relativedelta(hours=1)) + relativedelta(minutes=1),
        ...
        cls.so1before = cls.env['sale_custom.order'].create({
            'partner_id': cls.customer.id,
        ...
        cls.so1after = cls.env['sale_custom.order'].create({
            'partner_id': cls.customer.id,
            'website_id': cls.website1.id,
            'state': 'draft',
            'date_order': (now - relativedelta(minutes=30)) + relativedelta(minutes=1),
        ...
        cls.so2before = cls.env['sale_custom.order'].create({
            'partner_id': cls.customer.id,
        ...
        cls.so2after = cls.env['sale_custom.order'].create({
            'partner_id': cls.customer.id,
            'website_id': cls.website2.id,
            'state': 'draft',
            'date_order': (now - relativedelta(hours=24)) + relativedelta(minutes=1),
        ...
        cls.so2before_but_public = cls.env['sale_custom.order'].create({
            'partner_id': cls.public_partner.id,
            'website_id': cls.website2.id,
            'state': 'draft',
            'date_order': (now - relativedelta(hours=24)) - relativedelta(minutes=1),
        ...
        cls.so1before_but_other_public = cls.env['sale_custom.order'].create({
            'partner_id': cls.public_partner.id,
            'website_id': cls.website1.id,
            'state': 'draft',
            'date_order': (now - relativedelta(minutes=30)) - relativedelta(minutes=1),
    ...
    def test_website_sale_abandoned_cart_email(self):
        ...
        abandoned_sale_order = self.env['sale_custom.order'].create({
        ...
        self.assertTrue(abandoned_sale_order_custom.is_abandoned_cart)

        self.assertTrue(self.send_mail_patched(abandoned_sale_order_custom.id))

        # Test that no mail is sent if the partner has no email address.
        self.customer.email = False
        self.env['sale_custom.order'].create({
            'partner_id': self.customer.id,
            'website_id': website.id,
            'state': 'draft',
        ...
        self.assertFalse(self.send_mail_patched(abandoned_sale_order_custom.id))
        ...
        transaction = self.env['payment.transaction'].create({
            ...
            'partner_id': self.customer.id,
            'reference': abandoned_sale_order_custom.name,
            'amount': abandoned_sale_order_custom.amount_total,
            'state': 'error',
            'currency_id': self.env.ref('base.EUR').id,

        })
        abandoned_sale_order_custom.transaction_ids += transaction
        self.assertFalse(self.send_mail_patched(abandoned_sale_order_custom.id))
