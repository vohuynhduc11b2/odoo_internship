...
from odoo.addons.website_sale_custom.controllers.delivery import (
...
class TestWebsiteSaleDeliveryExpressCheckoutFlows(BaseUsersCommon, WebsiteSaleCommon, HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.sale_order_custom.partner_id = cls.public_partner.id
    ...
    def test_express_checkout_public_user_shipping_address_change(self):
        ...
        session['sale_order_id'] = self.sale_order_custom.id
        ...
        with patch(
            ...
            new_partner = self.sale_order_custom.partner_shipping_id
            self.assertNotEqual(new_partner, self.website.user_id.partner_id)
            self.assertTrue(new_partner.name.endswith(self.sale_order_custom.name))
    ...
    def test_express_checkout_public_user_shipping_address_change_twice(self):
        ...
        with patch(
            ...
            self.assertEqual(new_partner.id, self.sale_order_custom.partner_shipping_id.id)
            self.assertPartnerShippingValues(
                new_partner,
    ...
    def test_express_checkout_registered_user_exisiting_shipping_address_change(self):
        ...
        """
        self.sale_order_custom.partner_id = self.user_demo.partner_id.id
        ...
        with patch(
            ...
            self.assertEqual(self.sale_order_custom.partner_id.id, self.user_demo.partner_id.id)
    ...
    def test_express_checkout_partial_delivery_address_context_key(self):
        ...
        self.assertTrue(sale_order_custom._context.get('express_checkout_partial_delivery_address'))
    ...
    def test_express_checkout_registered_user_with_shipping_option_new_address(self):
        ...
        with patch(
            ...
            self.assertEqual(self.sale_order_custom.partner_shipping_id, self.user_demo.partner_id)
            ...
            self.assertNotEqual(
                self.sale_order_custom.partner_shipping_id.id, self.user_demo.partner_id.id
            )
            self.assertFalse(
                self.sale_order_custom.partner_shipping_id.name.endswith(self.sale_order_custom.name)
            )
