...
from odoo.addons.website_sale_custom.controllers.combo_configurator import (
    WebsiteSaleComboConfiguratorController,
)
from odoo.addons.website_sale_custom.controllers.main import WebsiteSale
from odoo.addons.website_sale_custom.controllers.payment import PaymentPortal
from odoo.addons.website_sale_custom.models.product_template import ProductTemplate
...
class TestWebsiteSaleCart(BaseUsersCommon, ProductAttributesCommon, WebsiteSaleCommon):
    ...
    def test_update_cart_before_payment(self):
        ...
        with MockRequest(self.product.with_user(self.public_user).env, website=website):
            ...
            sale_order_custom.access_token = 'test_token'
            old_amount = sale_order_custom.amount_total
            self.WebsiteSaleController.cart_update_json(product_id=self.product.id, add_qty=1)
            # Try processing payment with the old amount
            with self.assertRaises(UserError):
                PaymentPortal().shop_payment_transaction(sale_order_custom.id, sale_order_custom.access_token, amount=old_amount)
    ...
    def test_check_order_delivery_before_payment(self):
        ...
        with MockRequest(self.product.with_user(self.public_user).env, website=website):
            ...
            with self.assertRaises(ValidationError):
                PaymentPortal().shop_payment_transaction(sale_order_custom.id, sale_order_custom.access_token)
    ...
    def test_update_cart_zero_qty(self):
        ...
        with MockRequest(self.product.with_user(portal_user).env, website=website):
            ...
            self.WebsiteSaleController.cart_update_json(product_id=self.product.id, add_qty=1)
            sale_order = website.sale_get_order()
            self.assertEqual(sale_order_custom.amount_untaxed, 1000.0)

            # remove the product from the cart
            self.WebsiteSaleController.cart_update_json(product_id=self.product.id, line_id=sale_order_custom.order_line.id, set_qty=0)
            self.assertEqual(sale_order_custom.amount_total, 0.0)
            self.assertEqual(sale_order_custom.order_line, SaleOrderLine)
    ...
    def test_unpublished_accessory_product_visibility(self):
        ...
        with MockRequest(self.product.with_user(self.public_user).env, website=self.website.with_user(self.public_user)):
            self.WebsiteSaleController.cart_update_json(product_id=self.product.id, add_qty=1)
            sale_order = website.sale_get_order()
            self.assertEqual(len(sale_order_custom._cart_accessories()), 0)
    ...
    def test_cart_new_fpos_from_geoip(self):
        ...
        with MockRequest(website.env, website=website, country_code='BE'):
            ...
            sale_order = website.sale_get_order()
            self.assertEqual(
                sale_order_custom.fiscal_position_id, fpos_be,
    ...
    def test_checkout_no_delivery_method_available(self):
        ...
        with (MockRequest(self.product.with_user(portal_user).env, website=website), patch(
            'odoo.addons.website_sale_custom.models.sale_order_custom.SaleOrder._get_preferred_delivery_method',
