...
class WebsiteSaleCommon(ProductCommon, DeliveryCommon):
    ...
    def setUpClass(cls):
        ...
        cls.empty_cart = cls.env['sale_custom.order'].create({
            'partner_id': cls.partner.id,
            'website_id': cls.website.id,
        })
        cls.cart = cls.env['sale_custom.order'].create({
            'partner_id': cls.partner.id,
            'website_id': cls.website.id,
            'order_line': [
                Command.create({
    ...
    def _create_so(self, **values):
        ...
        return self.env['sale_custom.order'].create(dict(default_values, **values))
