...
export class ReorderConfirmationDialog extends ConfirmationDialog {
    static template = "website_sale_custom.ReorderConfirmationDialog";
}

export class ReorderDialog extends Component {
    static template = "website_sale_custom.ReorderModal";
    static props = {
        close: Function,
        orderId: Number,
        accessToken: String,
    ...
    async loadProductCombinationInfo(product) {
        for (const comboItem of product.selected_combo_items) {
            comboItem.combinationInfo = await rpc("/website_sale_custom/get_combination_info", {
        ...
        product.combinationInfo = await rpc("/website_sale_custom/get_combination_info", {
    ...
    async addProductsToCart() {
        for (const product of this.content.products) {
            ...
            if (product.selected_combo_items.length) {
                await rpc("/website_sale_custom/combo_configurator/update_cart", {
