...
options.registry.WebsiteSaleProductsItem = options.Class.extend({
    ...
    async _renderCustomXML(uiFragment) {
        ...
        Object.values(this.ribbons).forEach(ribbon => {
            $select.append(renderToElement('website_sale_custom.ribbonSelectItem', {
                ribbon,
...
options.registry.WebsiteSaleProductPage = options.Class.extend({
    ...
    async setImageWidth(previewMode, widgetValue, params) {
        ...
        if (zoomOption && widgetValue === "100_pc") {
            const defaultZoomOption = "website_sale_custom.product_picture_magnify_click";
    ...
    async setImageLayout(previewMode, widgetValue, params) {
        ...
        if (zoomOption) {
            ...
            let defaultZoomOption = widgetValue === "grid" ? "website_sale_custom.product_picture_magnify_click" : "website_sale_custom.product_picture_magnify_hover";
            if (imageWidthOption === "100_pc" && defaultZoomOption === "website_sale_custom.product_picture_magnify_hover") {
                defaultZoomOption = "website_sale_custom.product_picture_magnify_click";
            }
            await this._customizeWebsiteData(defaultZoomOption, { possibleValues: zoomOption._methodsParams.optionsPossibleValues["customizeWebsiteViews"] }, true);
        }
