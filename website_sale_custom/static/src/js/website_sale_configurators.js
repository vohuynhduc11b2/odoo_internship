...
import { WebsiteSale } from '@website_sale_custom/js/website_sale';
import wSaleUtils from '@website_sale_custom/js/website_sale_utils';
...
WebsiteSale.include({
    ...
    async _openDialog(isOnProductPage) {
        const { combos, show_quantity, ...remainingData } = await rpc(
            '/website_sale_custom/combo_configurator/get_data',
            {
        ...
        const shouldShowProductConfigurator = await rpc(
            '/website_sale_custom/should_show_product_configurator',
            {
    ...
    _openProductConfigurator(isOnProductPage, showQuantity) {
        this.call('dialog', 'add', ProductConfiguratorDialog, {
            ...
            save: async (mainProduct, optionalProducts, options) => {
                ...
                const values = await rpc('/website_sale_custom/product_configurator/update_cart', {
    ...
    async addComboProductToCart(comboProductData, selectedComboItems, remainingData, options) {
        ...
        const values = await rpc('/website_sale_custom/combo_configurator/update_cart', {
