...
var VariantMixin = {
    ...
    _getCombinationInfo: function (ev) {
        ...
        return rpc('/website_sale_custom/get_combination_info', {
    ...
    _onChangeCombination: function (ev, $parent, combination) {
        ...
        var self = this;
        var $price_holder = $parent.find(".oe_price:first");
        ...
        if (combination.uc_wait_sales_price) {
            $price_holder.removeClass('decimal_precision').removeAttr('data-precision').text("Giá liên hệ");
            $default_price.parent().addClass('d-none');
            $compare_price.addClass("d-none");
        } else {
            $price_holder.addClass('decimal_precision').attr('data-precision', precision);
            if ($price.length) {
                $price.text(self._priceToStr(combination.price, precision));
            } else {
                $price_holder.text(self._priceToStr(combination.price, precision));
            }
        ...
        } else if (combination.has_discounted_price && !combination.compare_list_price) {
            $default_price
                .closest('.oe_website_sale')
                .addClass("discount");
        ...
        const isOmsProductDetail = Boolean($parent.closest('.oms_product_detail').length);
        ...
        if (!isOmsProductDetail && (!combination.product_id ||
