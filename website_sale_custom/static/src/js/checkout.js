...
publicWidget.registry.WebsiteSaleCheckout = publicWidget.Widget.extend({
    ...
    events: {
        ...
        'change #oms_delivery_note': '_saveCheckoutNotes',
        'click a[name="website_sale_main_button"]': '_saveCheckoutNotesBeforeContinue',
    },
    ...
    async start() {
        ...
        this._boundSaveCheckoutNotesBeforeContinue = this._saveCheckoutNotesBeforeContinue.bind(this);
        this.mainButton?.addEventListener('click', this._boundSaveCheckoutNotesBeforeContinue);
    ...
    destroy() {
        this.mainButton?.removeEventListener('click', this._boundSaveCheckoutNotesBeforeContinue);
    ...
    async _saveCheckoutNotes() {
        const noteInput = this.el.querySelector('#oms_delivery_note');
        if (!noteInput) {
            return;
        }
        await rpc('/shop/save_checkout_notes', {note: noteInput.value || ''});
    },

    async _saveCheckoutNotesBeforeContinue(ev) {
        const noteInput = this.el.querySelector('#oms_delivery_note');
        if (!noteInput) {
            return;
        }
    ...
    async _setPickupLocation(pickupLocationData) {
        await rpc('/website_sale_custom/set_pickup_location', {pickup_location_data: pickupLocationData});
    },
