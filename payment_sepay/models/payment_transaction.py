...
class PaymentTransaction(models.Model):
    ...
    uc_deposit_applied = fields.Boolean(copy=False, default=False)
    ...
    def create(self, vals_list):
        ...
        for tx in txs.sudo():
            if tx.provider_code == "sepay" and tx.operation not in ("online_redirect", "validation"):
                try:
    ...
    def _get_specific_processing_values(self, processing_values):
        ...
        try:
            ...
            vals = {
                "uc_is_deposit": bool(is_deposit),
                "uc_deposit_percent": float(percent or 30.0),
            }
            ...
            _logger.info(
                ...
                self.reference, self.state, self.operation, mode,
    ...
    def _sepay_build_redirect_form_html(self, processing_values=None):
        self.ensure_one()
        rendering_vals = self._get_specific_rendering_values(processing_values or {})
        html = self.env["ir.ui.view"]._render_template(
            "payment_sepay.sepay_redirect_form",
            rendering_vals,
        )
        return html, rendering_vals

    def _get_processing_values(self):
        self.ensure_one()
        pv = super()._get_processing_values()
    
        if self.provider_code != "sepay":
            return pv
    
        try:
            if self.operation not in ("online_redirect", "validation"):
                self.sudo().write({"operation": "online_redirect"})
        except Exception:
            pass
        ...
        try:
            fixed_html, merged_vals = self._sepay_build_redirect_form_html(pv)
            pv.update(merged_vals)
            pv["redirect_form_html"] = fixed_html
        except Exception:
            _logger.exception("[SePay][PROC] cannot build redirect form tx=%s", self.reference)
    
        fixed_html_s = str(pv.get("redirect_form_html") or "")
        fixed_ok = (
            "<form" in fixed_html_s.lower()
            and 'id="o_payment_redirect_form"' in fixed_html_s.lower()
        )
    
        _logger.info(
            "[SePay][PROC] ref=%s op=%s state=%s mode=%s flow=%s fixed_ok=%s api_url=%s token=%s keys=%s",
            self.reference,
            self.operation,
            self.state,
            self.sepay_mode,
            ...
            fixed_ok,
            pv.get("api_url"),
            _mask(pv.get("access_token")),
            sorted(list(pv.keys()))[:60],
        )
                ...
                try:
                    for order in orders.filtered(lambda o: getattr(o, "website_id", False)):
                        ...
                        try:
                            ...
                            _logger.exception(
                                "[SePay] send approval failed order=%s tx=%s: %s",
                                order.display_name, tx.reference, e
                            )

                    tx.sudo().write({"sepay_sent_for_approval": True})
                    _logger.info(
                        "[SePay] finalize_post_processing approval ok_any=%s tx=%s mode=%s",
                        ok_any, tx.reference, mode
                    )
                except Exception:
